(() => {
  ...
  async function initializeAuthenticatedAdmin(){
    ...
    // Production baseline: no local preview configuration is loaded.
    // Registration, payment, eligibility and capacity data are server-authoritative.
  ...
  function renderRegistrations(){
    ...
    return `<div class="admin-panel-heading"><div><p class="eyebrow">Registration operations</p><h2>Registration Requests</h2><p>Review player details, verify level information, enforce same-level partner rules, and approve tournament entries.</p></div></div>
      ...
      ${records.length?records.map(recordCard).join(""):`<div class="empty-state"><strong>No registration requests yet.</strong>New player registrations will appear here automatically from Supabase.</div>`}
      </div>`;
  }
  ...
  function renderTrash(){
    if(!liveTrashLoaded&&!liveTrashLoading){
      setTimeoutclearLegacyRegistrationCaches();
  (async()=>{try{await loadTrashRecords({quiet:true});if(adminTab==="trash")renderAdmin()}catch(e){}},0);
    }
  ...
  function openRecord(reference){
    ...
    const issue=eligibility.levelMismatch
      ...
      ? `<div class="admin-warning-box"><strong>Validation still required</strong><p>Review the supporting information. Every player must be explicitly verified before approval. For no-DUPR players, review the club reference and playing history, then click Verify Requested Level or Set Different Level. DUPR screenshots must also be verified explicitly.</p></div>`
    ...
    const trashButton=$("[data-trash-record]",modal);if(trashButton)trashButton.onclick=()=>trashRegistration(reference);
    const pendingBtn=$("[data-set-pending]",modal);if(pendingBtn)pendingBtn.onclick=()=>{updateStatus(reference,"Pending");modal.close()};
    const denyBtn=$("[data-set-rejected]",modal);if(denyBtn)denyBtn.onclick=()=>{denyRegistration(reference);modal.close()};
    const approveBtn=$("[data-set-approved]",modal);if(approveBtn)approveBtn.onclick=()=>{updateStatus(reference,"Approved");modal.close()};
    const confirmBtn=$("[data-set-confirmed]",modal);if(confirmBtn)confirmBtn.onclick=()=>{updateStatus(reference,"Confirmed");modal.close()};
