(() => {
  ...
  const EXPECTED_API_CONTRACT = "2026.09.14.1";
  const REQUIRED_SCHEMA_VERSION = "2026.09.14.1";
  ...
  const PERMISSIONS = [
    ...
    {id:"access.manage",group:"System",label:"Manage access & roles",help:"Review organizer accounts and access policy.",directorOnly:true},
    {id:"users.invite",group:"System",label:"Invite organizer users",help:"Invite new organizer accounts with a temporary password."},
    {id:"users.revoke",group:"System",label:"Revoke organizer access",help:"Revoke or restore organizer access. Super Admin only."},
    {id:"users.password.manage",group:"System",label:"Reset other organizer passwords",help:"Issue a new temporary password to another organizer. Super Admin only."}
  ];
  ...
  // Registration Requests intentionally use one clear list workflow.
  const registrationView = "list";
  let searchTerm = "";
  ...
  let liveChangeCursor = 0;
  let liveSyncTimer = null;
  let liveSyncInFlight = false;
  let liveSyncLastAt = 0;
  ...
  const openPaymentMethodIds = new Set();
  let liveAdminAccounts = [];
  let liveAdminAccountsLoaded = false;
  let liveAdminAccountsLoading = false;
  let liveAdminAccessCapabilities = {
    currentUserId:null,
    currentEmail:"",
    isSuperAdmin:false,
    canInvite:false,
  ...
  let ownPasswordDialogForced = false;
  ...
  function currentRole(){
    ...
    return {
      ...
      isSuperAdmin:!!(supabaseAccessProfile?.is_super_admin||supabaseAccessProfile.role.is_super_admin),
  ...
  function recoveryHashPayload(){
    ...
    if(errorDescription){
      return {error:String(errorDescription).replace(/\+/g," ")};
    }
  ...
  function setRecoveryMode(enabled){
    ...
    const eyebrow=$("#adminAuthEyebrow");
    const title=$("#adminAuthTitle");
    ...
    if(enabled){
      ...
      if(title)title.textContent="Set a new Director password";
      if(description)description.textContent="Create a new password for this organizer account. You will return to the normal sign-in screen when the password has been updated.";
    }else{
      if(eyebrow)eyebrow.textContent="Secure organizer access";
      if(title)title.textContent="Sign in to Registration Control";
      if(description)description.textContent="Use an organizer account assigned to the Animo Pickleball Cup 2026 admin workspace.";
    }
  }

  function enterPasswordRecoveryMode(payload){
    passwordRecoverySession=payload;
    ...
    authStatus("Recovery link verified. Enter a new password below.","success");

    const input=$("#adminRecoveryPassword");
    if(input)setTimeout(()=>input.focus(),50);
  }

  async function updateRecoveredPassword(password){
    if(!passwordRecoverySession?.access_token){
      throw new Error("This recovery session is no longer available. Request a new password-reset email.");
    }

    await supabaseRequest("/auth/v1/user",{
      method:"PUT",
      body:{password},
      accessToken:passwordRecoverySession.access_token
    });
    ...
    try{
      ...
      console.warn("Recovery-session logout:",error);
    }
  ...
  async function loadMyAdminAccess({retry=true}={}){
    ...
    try{
      const profile=await supabaseRequest("/rest/v1/rpc/get_my_admin_access_v2",{
        method:"POST",
        body:{},
  ...
  function bindAuthUi(){
      ...
      form.addEventListener("submit",async event=>{
        ...
        try{
          ...
          if(profile?.must_change_password){
            bindGlobal();
            openOwnPasswordDialog({forced:true});
            return;
          }
    ...
    if(recoveryForm){
      recoveryForm.addEventListener("submit",async event=>{
        ...
        const password=String($("#adminRecoveryPassword")?.value||"");
        const confirmation=String($("#adminRecoveryPasswordConfirm")?.value||"");
        const button=$("#adminRecoveryButton");

        if(password.length<8){
          authStatus("Use a password with at least 8 characters.","error");
          return;
        }
        if(password!==confirmation){
          authStatus("The two password entries do not match.","error");
          return;
        }
        ...
        try{
          ...
          $("#adminRecoveryPassword").value="";
          $("#adminRecoveryPasswordConfirm").value="";

          lockAdmin("");
          authStatus("Password updated successfully. Sign in with your new password.","success");
          const email=$("#adminLoginEmail");
          if(email)setTimeout(()=>email.focus(),50);
        }catch(error){
          console.warn("Password recovery:",error);
          let message=error?.message||"Password could not be updated.";
          if(/expired|invalid.*token|jwt/i.test(message)){
            message="This password-reset link has expired or is no longer valid. Request a new recovery email.";
          }
          authStatus(message,"error");
        }finally{
          if(button){
    ...
    const recoveryToggle=$("#toggleRecoveryPassword");
    if(recoveryToggle){
      recoveryToggle.onclick=()=>{
        const password=$("#adminRecoveryPassword");
        const confirm=$("#adminRecoveryPasswordConfirm");
        if(!password||!confirm)return;
        const show=password.type==="password";
        password.type=show?"text":"password";
        confirm.type=show?"text":"password";
        recoveryToggle.textContent=show?"Hide":"Show";
        recoveryToggle.setAttribute("aria-label",show?"Hide new password":"Show new password");
      };
    }
  ...
  async function initializeAuthenticatedAdmin(){
    ...
    if(restored){
      bindGlobal();
      if(supabaseAccessProfile?.must_change_password){
        openOwnPasswordDialog({forced:true});
  ...
  function recordEligibility(r){
    ...
    if(partnerPending){
      return {levelKey:null,label:"Partner verification pending",rating:null,controller:"",manualReview:true,partnerPending:true,classificationPending:true,levelMismatch:false,higherLevelApplied:false,categoryPending:true,categoryKey:null,categoryLabel:"Pending partner",source:"",divisionId:null,readyForApproval:false};
    }
    if(r?.registrationType!=="pair"){
      return {levelKey:null,label:"Partner assignment pending",rating:null,controller:"",manualReview:true,partnerPending:false,classificationPending:true,levelMismatch:false,higherLevelApplied:false,categoryPending:true,categoryKey:null,categoryLabel:"Pending partner",source:"",divisionId:null,readyForApproval:false};
    }
    if(assessed.some(x=>!x.levelKey)){
      return {levelKey:null,label:"Level information required",rating:null,controller:"",manualReview:true,partnerPending:false,classificationPending:true,levelMismatch:false,higherLevelApplied:false,categoryPending:false,categoryKey:null,categoryLabel:"Pending",source:"",divisionId:null,readyForApproval:false};
    }
    const category=recordCategory(r);
    const higherLevelApplied=assessed[0].levelKey!==assessed[1].levelKey;
    const levelKey=assessed.reduce((best,current)=>levelRank(current.levelKey)>levelRank(best)?current.levelKey:best,assessed[0].levelKey);
    ...
    return {
      ...
      levelMismatch:false,
      higherLevelApplied,
      player1Level:assessed[0]?.levelKey||null,
      player2Level:assessed[1]?.levelKey||null,
      ...
      divisionId:exact?.id||null,
      readyForApproval,
      placementNote:higherLevelApplied?`${levelLabel(assessed[0].levelKey)} + ${levelLabel(assessed[1].levelKey)} → ${levelLabel(levelKey)}`:"Same-level pair"
    };
  }
  ...
  async function loadLiveRegistrations({quiet=false}={}){
    ...
    try{
      ...
      liveChangeCursor=Math.max(0,Number(response?.changeCursor||0));
      liveRegistrationLoaded=true;
      liveSyncLastAt=Date.now();
      updateRegistrationDataStatus(`LIVE SYNC · ${liveRegistrationRecords.length} registration${liveRegistrationRecords.length===1?"":"s"} loaded.`);
  ...
  function stopLiveRegistrationSync(){
    if(liveSyncTimer){
  ...
  function scheduleLiveRegistrationSync(delay=LIVE_SYNC_INTERVAL_MS){
    stopLiveRegistrationSync();
    if(!supabaseSession||!backendCompatible||!liveRegistrationLoaded||document.hidden)return;
    liveSyncTimer=setTimeout(()=>syncLiveRegistrationChanges(),Math.max(250,Number(delay)||LIVE_SYNC_INTERVAL_MS));
  }

  function liveSyncTimestamp(){
    try{
  ...
  async function syncLiveRegistrationChanges(){
    if(liveSyncInFlight||document.hidden||!supabaseSession||!backendCompatible||!liveRegistrationLoaded){
      scheduleLiveRegistrationSync();
      return;
    }

    liveSyncInFlight=true;
    try{
      const beforeRefs=new Set(liveRegistrationRecords.map(record=>record.reference));
      const response=await registrationAdminApi("admin-changes",{
        cursor:liveChangeCursor,
        limit:50
      });

      const changedRecords=Array.isArray(response?.records)?response.records:[];
      const removedRefs=Array.isArray(response?.removed)?response.removed:[];
      const changedRefs=new Set();
      let newCount=0;

      changedRecords.forEach(record=>{
        if(!record?.reference)return;
        if(!beforeRefs.has(record.reference))newCount++;
        changedRefs.add(record.reference);
        replaceLiveRecord(record);
      });

      removedRefs.forEach(reference=>{
        changedRefs.add(reference);
        removeLiveRegistration(reference);
      });

      liveChangeCursor=Math.max(liveChangeCursor,Number(response?.cursor||liveChangeCursor||0));
      liveSyncLastAt=Date.now();

      const count=liveRegistrationRecords.length;
      updateRegistrationDataStatus(`LIVE SYNC · ${count} registration${count===1?"":"s"} · checked ${liveSyncTimestamp()}`);

      if(changedRefs.size){
        // Only rerender screens whose contents depend on registration data.
        // Configuration/editor tabs are intentionally left untouched so an
        // incoming registration can never erase someone's unsaved form input.
        if(["registrations","reports"].includes(adminTab)){
          renderAdmin();
        }else if(adminTab==="trash"&&removedRefs.length){
          try{await loadTrashRecords({quiet:true})}catch{}
          renderAdmin();
        }

        const modal=$("#recordModal");
        if(modal?.open){
          const heading=modal.querySelector(".record-modal-reference");
          const modalReference=heading?.textContent?.trim()||"";
          if(modalReference&&changedRefs.has(modalReference)){
            showToast(`${modalReference} was updated on the server. Close and reopen the record to see the latest version.`);
          }
        }

        if(newCount>0){
          showToast(`${newCount} new registration${newCount===1?"":"s"} received`);
        }
      }

      if(response?.hasMore){
        scheduleLiveRegistrationSync(150);
      }else{
        scheduleLiveRegistrationSync();
      }
    }catch(error){
      console.warn("Live registration sync:",error);
      const message=String(error?.message||"temporary connection issue");

      if(/unknown action/i.test(message)){
        stopLiveRegistrationSync();
        updateRegistrationDataStatus(
          "LIVE SYNC UNAVAILABLE · registration-api does not have the admin-changes endpoint. Deploy the Production 1.0 Live Sync API.",
          true
        );
        showToast("Live sync requires the Production 1.0 Live Sync API");
        return;
      }

      updateRegistrationDataStatus(`LIVE SYNC RETRYING · ${message}`,true);
      scheduleLiveRegistrationSync(15000);
    }finally{
      liveSyncInFlight=false;
    }
  }

  function startLiveRegistrationSync(){
    if(!supabaseSession||!backendCompatible||!liveRegistrationLoaded)return;
    stopLiveRegistrationSync();
    if(document.hidden)return;
    // Run an immediate delta check after the initial full snapshot, then
    // continue with tiny cursor-based checks every six seconds.
    syncLiveRegistrationChanges();
  }
  ...
  function nextPaymentSortOrder(){
    const highest=Math.max(0,...livePaymentMethods.map(m=>Number(m?.sortOrder)||0));
    return highest+10;
  }

  function addLivePaymentMethod(){
    const id=`custom-${Date.now().toString(36)}`;
    livePaymentMethods.push({
      id,
      ...
      accountNumber:"",
      instructions:"",
      sortOrder:nextPaymentSortOrder(),
      qrUrl:"",
      qrDataUrl:"",
      removeQr:false
    });
    openPaymentMethodIds.add(id);
    renderAdmin();
    setTimeout(()=>{
      const input=document.querySelector(`[data-payment-method-accordion="${id}"] [data-payment-field="label"]`);
      if(input){
        input.focus();
        input.select?.();
      }
  ...
  function paymentAccordionOpen(m){
    return openPaymentMethodIds.has(String(m?.id||""))?" open":"";
  }
  ...
  function paymentMethodCard(m,i){
    ...
    const statusLabel=m.enabled?"Visible to players":"Hidden from players";
    const statusClass=m.enabled?"is-enabled":"is-hidden";
    const summaryDetail=[m.accountName,m.accountNumber].filter(Boolean).join(" · ")||"Not configured yet";
    const methodId=String(m.id||`payment-${i}`);

    return `<details class="payment-method-admin-card" data-payment-method-accordion="${esc(methodId)}"${paymentAccordionOpen(m)}>
      <summary class="payment-method-admin-summary">
        <div class="payment-method-summary-main">
          <span class="admin-index">${String(i+1).padStart(2,"0")}</span>
          <div class="payment-method-summary-copy">
            <strong data-payment-summary-label="${esc(methodId)}">${esc(m.label||"Payment method")}</strong>
            <small>${esc(summaryDetail)}</small>
          </div>
        </div>
        <div class="payment-method-summary-meta">
          <span class="payment-method-visibility ${statusClass}" data-payment-summary-status="${esc(methodId)}">${esc(statusLabel)}</span>
          <span class="payment-method-chevron" aria-hidden="true">⌄</span>
        </div>
      </summary>

      <div class="payment-method-admin-body">
        <div class="payment-method-visibility-row">
          <div>
            <strong>Player visibility</strong>
            <small>Enable this option only after the account details and instructions are ready.</small>
          </div>
          <label class="switch-label">
            <input type="checkbox" data-payment-index="${i}" data-payment-field="enabled" ${m.enabled?"checked":""}/>
            Enabled
          </label>
        </div>
        ...
        <div class="payment-method-admin-grid">
          <div class="payment-method-form">
            <label class="field"><span>Display name</span><input data-payment-index="${i}" data-payment-field="label" value="${esc(m.label||"")}" placeholder="e.g. UnionBank, GoTyme, PayPal"/></label>
            ...
            <label class="field"><span>Account number / payment ID</span><input data-payment-index="${i}" data-payment-field="accountNumber" value="${esc(m.accountNumber||"")}" placeholder="Required before enabling"/></label>
            ...
            <label class="field full"><span>Payment instructions</span><textarea data-payment-index="${i}" data-payment-field="instructions" placeholder="Tell players exactly how to pay and what reference to enter.">${esc(m.instructions||"")}</textarea></label>
          </div>
  ...
  function renderAdminPaymentMethods(){
    ...
    return `<div class="admin-panel-heading payment-method-page-heading">
      <div><p class="eyebrow">Payment collection</p><h2>Payment Methods</h2><p>Add any bank, e-wallet, transfer option, or other organizer-approved payment channel. Each method stays compact until you open it.</p></div>
      <div class="payment-method-page-actions">
        <button class="button button-ghost" id="addPaymentMethodButton" type="button">+ Add Payment Option</button>
        <button class="button button-primary" id="savePaymentMethodsButton" type="button">Save Payment Methods</button>
      </div>
    </div>
    <div class="payment-method-live-note">
      <div><strong>Live Supabase configuration</strong><span>${livePaymentMethods.filter(m=>m.enabled).length} of ${livePaymentMethods.length} methods visible to players.</span></div>
      <small>Open a payment method to edit its details.</small>
    </div>
    <div class="payment-method-admin-list">${livePaymentMethods.length?livePaymentMethods.map(paymentMethodCard).join(""):`<div class="empty-state"><strong>No payment methods configured.</strong><span>Add your first payment option to get started.</span></div>`}</div>`;
  }
  ...
  async function restoreRegistration(reference){
    ...
    try{
      const response=await registrationAdminApi("admin-restore",{reference});
      removeTrashRecord(reference);
      if(response?.record)replaceLiveRecord(response.record);
  ...
  function filteredRecords(){
    ...
    const rows=getRecords()
      ...
      .filter(r=>{
        ...
        if(registrationAttentionFilter==="higherTier")return !!e.higherLevelApplied;
        return true;
      });
  ...
  function renderRegistrations(){
    ...
    return `<div class="admin-panel-heading registration-heading">
      <div>
        <p class="eyebrow">Registration operations</p>
        <h2>Registration Requests</h2>
        <p>Review player details, verify level information, confirm higher-tier placement when player levels differ, and approve tournament entries.</p>
      </div>
    </div>
      ...
      <div class="registration-search-toolbar">
        ...
        <label class="field compact-filter registration-attention-filter"><span>Director queue</span><select id="attentionFilter">
          ...
          <option value="higherTier" ${registrationAttentionFilter==="higherTier"?"selected":""}>Higher-tier placement</option>
        ...
        <table class="registration-list-table">
          <thead>
            <tr>
              ...
              <th>Status</th>
              <th>Submitted</th>
              <th class="registration-list-actions-head">Next step</th>
            </tr>
          </thead>
          <tbody>
  ...
  function registrationListRow(r){
    ...
    const paymentStatus=String(r.payment?.status||"Submitted");
    const paymentVerified=paymentStatus.toLowerCase()==="verified";
    const proofAvailable=!!(r.payment?.proofAvailable||r.payment?.fileName);
    const eligibilityReady=!eligibility.classificationPending&&!eligibility.partnerPending&&!eligibility.categoryPending&&!eligibility.manualReview&&!!eligibility.divisionId;
    const currentStatus=workflowStatus(r.status);
    const submitted=r.submittedAt
      ? new Date(r.submittedAt).toLocaleString("en-PH",{dateStyle:"medium",timeStyle:"short"})
      : "—";

    let next={label:"Review record",detail:"Open the complete registration record.",focus:"eligibility",tone:"neutral"};
    const openIssue=(r.reviewIssues||[]).find(issue=>issue?.status==="Open");
    if(currentStatus==="Under Review"){
      if(openIssue){
        next={label:"Resolve action required",detail:openIssue.requiredAction||openIssue.publicMessage||"A registrant update is required.",focus:openIssue.scope==="payment"?"payment":"eligibility",tone:"warning"};
      }else if(!eligibilityReady){
        next={label:"Verify player levels",detail:"Eligibility or division placement still needs review.",focus:"eligibility",tone:"warning"};
      }else if(!proofAvailable){
        next={label:"Review payment",detail:"Payment proof is still missing.",focus:"payment",tone:"warning"};
      }else if(!paymentVerified){
        next={label:"Verify payment",detail:"Player checks are complete; payment is next.",focus:"payment",tone:"warning"};
      }else if(readiness.ready){
        next={label:"Approve registration",detail:"All required checks are complete.",focus:"approval",tone:"ready"};
      }
    }else if(currentStatus==="Approved"){
      next={label:"View approved record",detail:"No approval action is pending.",focus:"top",tone:"complete"};
    }else if(["Declined","Withdrawn"].includes(currentStatus)){
      next={label:"Review status",detail:"Open the record to reopen or review history.",focus:"top",tone:"neutral"};
    }

    const placementNote=eligibility.higherLevelApplied
      ? `<span class="review-placement-note">Higher-tier placement: ${esc(levelLabel(eligibility.levelKey))}</span>`
      : "";
    ...
    return `<tr class="registration-list-row registration-list-row-clickable" data-row-reference="${esc(r.reference)}" tabindex="0" role="button" aria-label="Open registration ${esc(r.reference)}">
      ...
      <td data-label="Division"><span class="registration-list-division">${esc(divs)}</span>${placementNote}</td>
      <td data-label="Review status">
        <div class="registration-review-readiness">
          <div class="review-checks" aria-label="Required checks">
            <span class="review-check ${eligibilityReady?"is-complete":"needs-action"}"><b>${eligibilityReady?"✓":"!"}</b> Player level</span>
            <span class="review-check ${paymentVerified?"is-complete":"needs-action"}"><b>${paymentVerified?"✓":"!"}</b> Payment</span>
          </div>
          <strong class="review-next-title">${esc(next.label)}</strong>
          <span class="review-next-detail">${esc(next.detail)}</span>
        </div>
      </td>
      <td data-label="Status">
        <span class="status-pill workflow-status workflow-status-${currentStatus.toLowerCase().replace(/\s+/g,"-")}">${esc(currentStatus)}</span>
      </td>
      <td data-label="Submitted"><span class="registration-list-date">${esc(submitted)}</span></td>
      <td class="registration-list-actions" data-label="Next step">
        <button class="button registration-next-action registration-next-action-${next.tone}" data-view-record="${esc(r.reference)}" data-focus-section="${esc(next.focus)}" type="button">${esc(next.label)}</button>
      </td>
    </tr>`;
  }
  ...
  function approvalReadiness(record){
    ...
    const openIssues=(record?.reviewIssues||[]).filter(issue=>issue?.status==="Open");
    if(openIssues.length)missing.push("Action Required items");
  ...
  function recordCard(r){
    ...
    const verificationLabel=eligibility.higherLevelApplied
      ?`Higher tier: ${levelLabel(eligibility.levelKey)}`
    ...
    return `<article
      ...
      <div class="director-card-body">
        ...
        <div class="director-decision-grid">
          ...
          <div class="director-decision-item ${eligibility.manualReview?"decision-warning":"decision-good"}">
  ...
  function paymentVerificationMarkup(reference,payment){
    ...
    if(!hasProof){
      return `<div class="payment-review-decision">
        <div class="payment-verification-state payment-verification-missing">
          <strong>Payment proof is missing</strong>
          <span>Notify the registrants if a replacement proof or other action is required.</span>
        </div>
        ${can("payments.verify")?`<button class="button button-flag-issue" data-flag-review-issue="payment" data-record="${esc(reference)}" type="button">Flag Issue & Notify</button>`:""}
      </div>`;
    }
    ...
    return `<div class="review-decision-actions">
      <button class="button button-verify-payment" data-verify-payment="${esc(reference)}" type="button">✓ Verify Payment</button>
      <button class="button button-flag-issue" data-flag-review-issue="payment" data-record="${esc(reference)}" type="button">⚠ Flag Issue</button>
    </div>`;
  }
  ...
  function verificationActions(reference,slot,v){
    ...
    if(v?.hasDupr==="Yes"){
      if(can("eligibility.verify")&&v?.duprProofStatus!=="Verified"){
        actions.push(`<button class="button button-verify-action" data-live-verify-dupr="${playerSlot}" data-record="${esc(reference)}" type="button">✓ Verify DUPR</button>`);
      }
      if(can("eligibility.verify")||can("eligibility.reclassify")){
        actions.push(`<button class="button button-flag-issue" data-flag-review-issue="eligibility" data-player-slot="${playerSlot}" data-record="${esc(reference)}" type="button">⚠ Review / Reclassify</button>`);
      }
    }

    if(v?.hasDupr==="No"){
      if(can("eligibility.verify")&&!v?.manualLevelApproved){
        actions.push(`<button class="button button-verify-action" data-live-verify-level="${playerSlot}" data-record="${esc(reference)}" type="button">✓ Verify Submitted Level</button>`);
      }
      if(can("eligibility.verify")||can("eligibility.reclassify")){
        actions.push(`<button class="button button-flag-issue" data-flag-review-issue="eligibility" data-player-slot="${playerSlot}" data-record="${esc(reference)}" type="button">⚠ Review / Reclassify</button>`);
      }
    ...
    return actions.length?`<div class="verification-actions review-decision-actions">${actions.join("")}</div>`:"";
  }

  function issueTypeLabel(value){
    const labels={
      incorrect_amount:"Incorrect payment amount",
      invalid_proof:"Invalid or unclear payment proof",
      wrong_payment_details:"Wrong payment details",
      partial_payment:"Partial payment / balance due",
      overpayment:"Overpayment",
      new_payment_proof:"New payment proof required",
      new_dupr_proof:"New DUPR proof required",
      additional_information:"Additional information required",
      reclassification_required:"Reclassification required",
      eligibility_concern:"Eligibility concern",
      other:"Other review issue"
    };
    return labels[value]||String(value||"Review issue").replaceAll("_"," ");
  }

  function openReviewIssues(record,scope,playerSlot=null){
    return (record?.reviewIssues||[]).filter(issue=>
      issue?.status==="Open"&&issue?.scope===scope&&(playerSlot==null||Number(issue?.playerSlot)===Number(playerSlot))
    );
  }

  function reviewIssueSummaryMarkup(record,scope,playerSlot=null){
    const issues=openReviewIssues(record,scope,playerSlot);
    if(!issues.length)return "";
    return `<div class="review-issue-stack">${issues.map(issue=>`
      <article class="review-issue-card">
        <div class="review-issue-head"><span>Action Required</span><strong>${esc(issueTypeLabel(issue.issueType))}</strong></div>
        <p>${esc(issue.publicMessage||"")}</p>
        ${issue.requiredAction?`<div class="review-required-action"><span>Registrant action</span><strong>${esc(issue.requiredAction)}</strong></div>`:""}
        <div class="review-issue-meta">${issue.notifiedAt?`Notification sent ${esc(new Date(issue.notifiedAt).toLocaleString("en-PH"))}`:"Notification pending or not delivered"}</div>
        ${can(scope==="payment"?"payments.verify":"eligibility.verify")?`<button class="text-button review-resolve-button" data-resolve-review-issue="${esc(issue.id)}" type="button">Mark resolved</button>`:""}
      </article>`).join("")}</div>`;
  }

  function issueDefaults(scope,type,record,playerSlot){
    const amountDue=Number(record?.payment?.amountDue)||reportExpectedFee(record)||0;
    const paid=Number(record?.payment?.amountPaid)||0;
    const balance=Math.max(0,amountDue-paid);
    const payment={
      incorrect_amount:["The submitted payment amount does not match the amount required for this registration.",`Please review the payment and settle the correct amount${balance?` (remaining balance: ${formatMoney(balance)})`:""}.`],
      invalid_proof:["The submitted proof of payment could not be validated from the uploaded file.","Please provide a clear and valid proof of payment showing the transaction details."],
      wrong_payment_details:["The submitted payment details do not match the transaction information we can verify.","Please review the payment reference, sender details, and payment information and provide the corrected details."],
      partial_payment:[`A partial payment was received. ${balance?`The remaining balance is ${formatMoney(balance)}.`:"A balance remains due."}`,`Please settle the remaining balance${balance?` of ${formatMoney(balance)}`:""} and provide the updated proof of payment.`],
      overpayment:["The amount received is higher than the amount required for this registration.","Please wait for the tournament administration to confirm how the excess payment will be handled."],
      new_payment_proof:["A replacement proof of payment is required before this registration can be approved.","Please provide a new, clear proof of payment to the tournament organizer."],
      other:["The tournament administration found an item in the payment submission that requires clarification.","Please review the organizer note and provide the requested clarification."]
    };
    const level={
      new_dupr_proof:["The submitted DUPR proof requires replacement or additional verification.","Please provide an updated, clear DUPR screenshot or supporting DUPR information."],
      additional_information:["Additional player information is required before the submitted playing level can be verified.","Please provide the additional information requested by the tournament administrator."],
      reclassification_required:["The submitted playing level requires organizer review and possible reclassification.","Please review the organizer notice. Final level and division placement remain subject to tournament approval."],
      eligibility_concern:["The tournament administration identified an eligibility item that must be resolved before approval.","Please provide the requested supporting information so the player eligibility review can continue."],
      other:["The tournament administration needs clarification regarding the submitted player level or eligibility information.","Please review the organizer note and provide the requested clarification."]
    };
    return (scope==="payment"?payment:level)[type]||(scope==="payment"?payment.other:level.other);
  }

  function ensureReviewIssueDialog(){
    let dialog=$("#reviewIssueDialog");
    if(dialog)return dialog;
    document.body.insertAdjacentHTML("beforeend",`<dialog id="reviewIssueDialog" class="review-issue-dialog"><form method="dialog" class="review-issue-modal" id="reviewIssueForm">
      <header><div><p class="eyebrow">Registration review</p><h2 id="reviewIssueTitle">Flag Issue</h2><p id="reviewIssueSubtitle">Save the review item and notify both registrants by email.</p></div><button type="button" class="modal-close" id="reviewIssueClose" aria-label="Close">×</button></header>
      <div id="reviewIssueCorrection" class="review-correction-panel" hidden></div>
      <label class="field"><span>Issue type</span><select id="reviewIssueType"></select></label>
      <label class="field"><span>Message to registrant</span><textarea id="reviewIssuePublicMessage" rows="4" required></textarea><small>This text is visible to the players and included in the email.</small></label>
      <label class="field"><span>Required action</span><textarea id="reviewIssueRequiredAction" rows="3" required></textarea></label>
      <label class="field"><span>Internal note</span><textarea id="reviewIssueInternalNote" rows="3" placeholder="Optional. Visible only to organizers."></textarea></label>
      <footer><button class="button button-ghost" id="reviewIssueCancel" type="button">Cancel</button><button class="button button-primary" id="reviewIssueSave" type="submit">Save & Notify Registrants</button></footer>
    </form></dialog>`);
    dialog=$("#reviewIssueDialog");
    $("#reviewIssueClose",dialog).onclick=()=>dialog.close();
    $("#reviewIssueCancel",dialog).onclick=()=>dialog.close();
    return dialog;
  }

  function openReviewIssueDialog(reference,scope,playerSlot=null){
    const record=findRegistrationRecord(reference);
    if(!record)return;
    const dialog=ensureReviewIssueDialog();
    const verification=playerSlot===2?record.verification2:record.verification1;
    const typeSelect=$("#reviewIssueType",dialog);
    const options=scope==="payment"
      ? [["incorrect_amount","Incorrect payment amount"],["invalid_proof","Invalid / unclear proof"],["wrong_payment_details","Wrong payment details"],["partial_payment","Partial payment / balance due"],["overpayment","Overpayment"],["new_payment_proof","Request new payment proof"],["other","Other"]]
      : verification?.hasDupr==="Yes"
        ? [["new_dupr_proof","Request new DUPR proof"],["additional_information","Request additional information"],["reclassification_required","Reclassification required"],["eligibility_concern","Eligibility concern"],["other","Other"]]
        : [["additional_information","Request additional information"],["reclassification_required","Reclassification required"],["eligibility_concern","Eligibility concern"],["other","Other"]];
    typeSelect.innerHTML=options.map(([value,label])=>`<option value="${value}">${label}</option>`).join("");
    $("#reviewIssueTitle",dialog).textContent=scope==="payment"?"Flag Payment Issue":`Review Player ${playerSlot} Level`;
    $("#reviewIssueSubtitle",dialog).textContent="The registration remains under review. Saving this item emails both registered players and displays Action Required in their status tracker.";

    const correction=$("#reviewIssueCorrection",dialog);
    correction.hidden=true;
    correction.innerHTML="";
    if(scope==="eligibility"&&can("eligibility.reclassify")){
      correction.hidden=false;
      correction.innerHTML=`<div><strong>Need to correct the record now?</strong><span>Use an administrative correction instead of asking the player to resubmit information.</span></div><button class="button button-ghost" id="reviewIssueCorrectionButton" type="button">${verification?.hasDupr==="Yes"?"Correct DUPR Rating":"Assign Different Level"}</button>`;
      $("#reviewIssueCorrectionButton",correction).onclick=()=>{
        dialog.close();
        if(verification?.hasDupr==="Yes")liveCorrectDupr(reference,Number(playerSlot));
        else liveChangeNoDuprLevel(reference,Number(playerSlot));
      };
    }

    const syncCopy=()=>{
      const [message,action]=issueDefaults(scope,typeSelect.value,record,playerSlot);
      $("#reviewIssuePublicMessage",dialog).value=message;
      $("#reviewIssueRequiredAction",dialog).value=action;
    };
    typeSelect.onchange=syncCopy;
    syncCopy();
    $("#reviewIssueInternalNote",dialog).value="";

    const form=$("#reviewIssueForm",dialog);
    form.onsubmit=async(event)=>{
      event.preventDefault();
      const save=$("#reviewIssueSave",dialog);
      save.disabled=true;save.textContent="Saving & notifying…";
      try{
        const response=await registrationAdminApi("admin-review-issue",{
          reference,
          scope,
          playerSlot,
          issueType:typeSelect.value,
          publicMessage:$("#reviewIssuePublicMessage",dialog).value,
          requiredAction:$("#reviewIssueRequiredAction",dialog).value,
          internalNote:$("#reviewIssueInternalNote",dialog).value,
          metadata:scope==="payment"?{amountDue:record?.payment?.amountDue??null,amountSubmitted:record?.payment?.amountPaid??null}:{}
        });
        if(response?.record)replaceLiveRecord(response.record);
        dialog.close();
        renderAdmin();
        openRecord(reference,scope==="payment"?"payment":"eligibility");
        const sent=Number(response?.email?.sent||0), failed=Number(response?.email?.failed||0);
        showToast(sent?`Action Required saved · ${sent} email${sent===1?"":"s"} sent${failed?` · ${failed} failed`:""}`:"Action Required saved · email delivery needs attention");
      }catch(error){
        showToast(error?.message||"Action Required item could not be saved");
      }finally{
        save.disabled=false;save.textContent="Save & Notify Registrants";
      }
    };
    dialog.showModal();
  }
  ...
  function openRecord(reference,focusSection=""){
    ...
    const playerBlock=(label,p,v,slot)=>{
      ...
      return `<section class="record-detail-section player-record-card">
        ...
        ${reviewIssueSummaryMarkup(r,"eligibility",playerSlot)}
      </section>`;
    };

    const issue=eligibility.higherLevelApplied
      ? `<div class="admin-info-box"><strong>Higher-tier placement applied</strong><p>The players have different assessed levels. Under the current tournament rule, the team is placed in <b>${esc(levelLabel(eligibility.levelKey))}</b>, the higher tier. This registration remains subject to normal admin approval.</p></div>`
      : eligibility.classificationPending
      ...
      : `<div class="record-ready-box"><strong>✓ Player eligibility complete</strong><span>${eligibility.higherLevelApplied?`Higher-tier placement is resolved to ${esc(levelLabel(eligibility.levelKey))}.`:"Player level placement and division are resolved."}</span></div>`;

    const paymentHeld=eligibility.classificationPending||eligibility.partnerPending||eligibility.categoryPending||!eligibility.divisionId;
    ...
    $("#recordModalBody").innerHTML=`
      <div class="record-modal-shell">
        ...
        <div class="record-modal-scroll">
          <section class="record-detail-section eligibility-record-section" id="recordEligibilitySection">
            <div class="record-section-title">
            ...
            <div class="record-detail-grid overview-detail-grid">
              ...
              ${detail("Level placement",eligibility.classificationPending?"Pending":eligibility.higherLevelApplied?`Higher tier → ${levelLabel(eligibility.levelKey)}`:"Same-level pair")}
          ...
          <section class="record-detail-section payment-record-section" id="recordPaymentSection">
            <div class="record-section-title">
            ...
            <div class="record-payment-layout">
              <div>
                ...
                ${reviewIssueSummaryMarkup(r,"payment")}
          ...
          ${currentWorkflowStatus==="Under Review"?`
            <section class="approval-readiness ${readiness.ready?"is-ready":"needs-work"}" id="recordApprovalSection">
              <div>
    ...
    const modalScroll=$(".record-modal-scroll",modal);
    const syncRecordModalViewport=()=>{
      if(!modal?.open||!modalScroll)return;
      const modalHeight=modal.getBoundingClientRect().height;
      const modalTop=$(".record-modal-top",modal);
      const modalFooter=$(".record-modal-actions",modal);
      const topHeight=modalTop?.getBoundingClientRect().height||0;
      const footerHeight=modalFooter?.getBoundingClientRect().height||0;
      const available=Math.max(180,Math.floor(modalHeight-topHeight-footerHeight));
      modalScroll.style.height=`${available}px`;
      modalScroll.style.maxHeight=`${available}px`;
      modalScroll.style.overflowY="auto";
    };
    if(modal._recordModalResizeHandler){
      window.removeEventListener("resize",modal._recordModalResizeHandler);
    }
    modal._recordModalResizeHandler=syncRecordModalViewport;
    window.addEventListener("resize",syncRecordModalViewport,{passive:true});
    requestAnimationFrame(()=>{
      syncRecordModalViewport();
      requestAnimationFrame(syncRecordModalViewport);
    });
    setTimeout(syncRecordModalViewport,80);

    if(modalScroll){
      modalScroll.scrollTop=0;
      modalScroll.tabIndex=0;
      if(focusSection&&focusSection!=="top"){
        const targetId=focusSection==="payment"?"#recordPaymentSection":focusSection==="approval"?"#recordApprovalSection":"#recordEligibilitySection";
        requestAnimationFrame(()=>{
          syncRecordModalViewport();
    ...
    $$('[data-flag-review-issue]',modal).forEach(btn=>btn.onclick=()=>openReviewIssueDialog(
      btn.dataset.record||r.reference,
      btn.dataset.flagReviewIssue,
      btn.dataset.playerSlot?Number(btn.dataset.playerSlot):null
    ));
    $$('[data-resolve-review-issue]',modal).forEach(btn=>btn.onclick=async()=>{
      if(!confirm("Mark this Action Required item as resolved?"))return;
      try{
        const response=await registrationAdminApi("admin-review-issue-resolve",{issueId:btn.dataset.resolveReviewIssue,resolutionNote:"Resolved by organizer from Registration Control."});
        if(response?.record)replaceLiveRecord(response.record);
        renderAdmin();openRecord(r.reference,focusSection||"eligibility");showToast("Action Required item resolved");
      }catch(error){showToast(error?.message||"Review item could not be resolved")}
    });
    ...
    if(saveResolution)saveResolution.onclick=async()=>{
      ...
      try{
        const response=await registrationAdminApi("admin-payment-resolution",{
  ...
  function reconcileRecordPlacement(record){
    ...
    record.paymentHeld=!!(e.classificationPending||e.partnerPending||e.categoryPending||!e.divisionId);
    record.eligibilitySnapshot={
      ...
      levelMismatch:false,
      higherLevelApplied:!!e.higherLevelApplied,
      categoryPending:!!e.categoryPending,
      categoryKey:e.categoryKey||null,
      categoryLabel:e.categoryLabel||null,
      rule:"different partner levels are allowed; the higher assessed level controls team placement; gender is Male/Female only and determines category automatically; no-DUPR players may request a level subject to organizer validation"
    };
    return record;
  }
  ...
  async function liveVerifyPlayer(reference,playerSlot,verificationAction,payload={}){
    ...
    try{
      ...
      if(response?.synchronization?.higherLevelApplied){
        const effective=response?.synchronization?.effectiveLevel;
        showToast(`Player verified · higher-tier placement${effective?` → ${levelLabel(effective)}`:" applied"}`);
      }else{
  ...
  async function updateStatus(reference,status,options={}){
    ...
    try{
      ...
      // The API returns the authoritative changed record. Replace only that
      // record instead of re-downloading the full registration dataset.
      if(response?.record)replaceLiveRecord(response.record);
  ...
  // =========================================================
  // REPORTS & INSIGHTS — Tournament Director reporting workspace
  // =========================================================
  const reportPalette=["#14b8a6","#22c55e","#84cc16","#eab308","#38bdf8","#a78bfa","#f97316","#f43f5e","#06b6d4","#10b981","#f59e0b","#8b5cf6"];
  function reportElementColor(reportKey,row,index){
    const label=String(row?.label||"");
    if(reportKey==="financial"){
      const financialColors={
        "Expected active fees":"#16b8ad",
        "Payment amounts submitted":"#22c55e",
        "Verified collected":"#7ccf00",
        "Awaiting verification":"#f0b500",
        "Uncollected / no amount submitted":"#df5d5d",
        "Verified balance outstanding":"#9b7bea"
      };
      if(financialColors[label])return financialColors[label];
    }
    return reportPalette[index%reportPalette.length];
  }

  function reportDrillAttrs(reportKey,label){
    if(!reportKey)return "";
    return ` data-report-drill-key="${esc(reportKey)}" data-report-drill-label="${esc(String(label??""))}" role="button" tabindex="0" aria-label="View breakdown for ${esc(String(label??"report item"))}"`;
  }
  ...
  function reportKpis(records){
    ...
    const needsReview=records.filter(r=>reportActionIssues(r).length).length;
    const duprPlayers=players.filter(p=>p.hasDupr==="Yes").length;
    const finance=reportFinanceSnapshot(records);
    ...
    return {
      ...
      paid:finance.submitted,
      verified:finance.verified,
      awaiting:finance.awaiting,
      balance:finance.verifiedOutstanding,
  ...
  function reportIsActiveRecord(record){
    return !["Declined","Withdrawn","Cancelled"].includes(workflowStatus(record?.status));
  }

  function reportPaymentExpected(record){
    const direct=Number(record?.payment?.amountDue);
    return Number.isFinite(direct)&&direct>0?direct:reportExpectedFee(record);
  }

  function reportPaymentSubmitted(record){
    const value=Number(record?.payment?.amountPaid);
    return Number.isFinite(value)&&value>0?value:0;
  }

  function reportPaymentVerifiedAmount(record){
    return String(record?.payment?.status||"").toLowerCase()==="verified"?reportPaymentSubmitted(record):0;
  }

  function reportPaymentState(record){
    const payment=record?.payment||{};
    const status=String(payment.status||"").trim();
    const verified=status.toLowerCase()==="verified";
    const proof=!!(payment.proofAvailable||payment.fileName);
    const paid=reportPaymentSubmitted(record)>0;
    if(verified)return "Verified";
    if(paid||proof)return "Awaiting verification";
    return reportIsActiveRecord(record)?"Outstanding":"Not applicable";
  }

  function reportPaymentProofState(record){
    const payment=record?.payment||{};
    if(payment.proofAvailable||payment.fileName)return "Uploaded";
    return "Missing";
  }

  function reportPlayerLevel(verification){
    const assessment=playerAssessment(verification||{});
    return assessment.levelKey?levelLabel(assessment.levelKey):(assessment.label||"Pending");
  }

  function reportActionIssues(record){
    const issues=[];
    const e=recordEligibility(record);
    if(e.partnerPending)issues.push("Partner information pending");
    if(e.classificationPending)issues.push("Classification pending");
    if(e.categoryPending)issues.push("Category pending");
    if(e.manualReview)issues.push("Organizer level validation required");
    if(reportIsActiveRecord(record)){
      if(reportPaymentProofState(record)==="Missing")issues.push("Payment proof missing");
      if(reportPaymentState(record)==="Awaiting verification")issues.push("Payment verification required");
      if(reportPaymentState(record)==="Outstanding")issues.push("Payment outstanding");
      if(workflowStatus(record.status)==="Under Review"&&!issues.some(x=>x.includes("validation")))issues.push("Registration approval pending");
    }
    return [...new Set(issues)];
  }

  function reportFinanceSnapshot(records){
    const active=records.filter(reportIsActiveRecord);
    const expected=active.reduce((s,r)=>s+reportPaymentExpected(r),0);
    const submitted=active.reduce((s,r)=>s+reportPaymentSubmitted(r),0);
    const verified=active.reduce((s,r)=>s+reportPaymentVerifiedAmount(r),0);
    const awaiting=active.filter(r=>reportPaymentState(r)==="Awaiting verification").reduce((s,r)=>s+reportPaymentSubmitted(r),0);
    const uncollected=Math.max(expected-submitted,0);
    const verifiedOutstanding=Math.max(expected-verified,0);
    const overpaid=active.reduce((s,r)=>s+Math.max(reportPaymentSubmitted(r)-reportPaymentExpected(r),0),0);
    const refunds=active.reduce((s,r)=>s+(Number(r.payment?.refundAmount)||0),0);
    const adjustments=active.reduce((s,r)=>s+(Number(r.payment?.adjustmentAmount)||0),0);
    return {
      active,
      expected,
      submitted,
      ...
      refunds,
      adjustments,
      collectionRate:expected?Math.round(verified/expected*100):0,
      verifiedCount:active.filter(r=>reportPaymentState(r)==="Verified").length,
      awaitingCount:active.filter(r=>reportPaymentState(r)==="Awaiting verification").length,
      outstandingCount:active.filter(r=>reportPaymentState(r)==="Outstanding").length
    };
  }

  function reportDefinitions(){
    const definitions=[
      ["executiveSummary","Executive tournament summary","Director"],
      ["registrationSummary","Registration master report","Registration"],
      ...
      ["capacity","Division & capacity report","Operations"],
      ["tournamentRoster","Final tournament roster","Operations"],
      ["jersey","Jersey print list","Operations"],
      ["detail","Full registration detail","Operations"],
      ["playersTeams","Player & team report","Players"],
      ...
      ["actionRequired","Outstanding actions report","Fairness & verification"],
      ["dupr","Players with / without DUPR","Fairness & verification"],
      ["ratingBands","DUPR rating distribution","Fairness & verification"],
      ["financial","Financial report","Finance"],
      ["paymentReconciliation","Payment reconciliation report","Finance"],
      ["paymentStatus","Payment status","Finance"],
      ["paymentMethod","Payments by method","Finance"],
      ["revenueMethod","Verified revenue by payment method","Finance"],
      ["paymentDivision","Verified revenue by division","Finance"],
      ["auditTrail","Activity / audit report","Governance"],
      ["emailDelivery","Email / notification report","Governance"]
    ];
    return definitions.filter(([key,,group])=>{
      if(group==="Finance"&&!can("reports.financial"))return false;
      if(key==="auditTrail"&&!can("access.manage"))return false;
      if(key==="emailDelivery"&&!can("notifications.manage")&&!can("access.manage"))return false;
      return true;
    });
  }
  ...
  function reportDefinition(key,records){
    ...
    if(key==="executiveSummary"){
      ...
      const f=reportFinanceSnapshot(records);
      const approved=records.filter(r=>workflowStatus(r.status)==="Approved");
      const activeCapacity=(config.divisions||[]).filter(d=>d.enabled).reduce((sum,d)=>sum+(Number(d.capacity)||0),0);
      const rows=[
        ["Registration requests",k.registrations,"Filtered active registration records"],
        ["Known players",k.players,"Player profiles in the current scope"],
        ["Approved teams",approved.length,activeCapacity?`${approved.length} of ${activeCapacity} configured team slots`:"Approved registrations"],
        ["Needs attention",records.filter(r=>reportActionIssues(r).length).length,"Entries with registration, eligibility, or payment actions outstanding"],
        ["Expected registration fees",formatMoney(f.expected),"Active registrations only"],
        ["Verified collected",formatMoney(f.verified),`${f.collectionRate}% of expected active fees`],
        ["Awaiting payment verification",formatMoney(f.awaiting),`${f.awaitingCount} registration${f.awaitingCount===1?"":"s"}`],
        ["Verified balance outstanding",formatMoney(f.verifiedOutstanding),"Expected active fees minus verified collections"]
      ];
      return {
        title:"Executive tournament summary",
        description:"Tournament-director snapshot of registration progress, operational workload, capacity, and verified collections.",
        defaultChart:"table",
        tableOnly:true,
        tableHeaders:["Indicator","Current value","Director note"],
        tableRows:rows
      };
    }
    if(key==="registrationSummary"){
      const rows=records.map(r=>{
        const e=recordEligibility(r);
        const people=[fullName(r.player1),r.registrationType==="pair"?fullName(r.player2):""].filter(Boolean).join(" / ");
        const issues=reportActionIssues(r);
        return [
          r.reference||"—",
          ...
          reportPaymentState(r),
          formatMoney(reportPaymentSubmitted(r)),
          issues.length?issues.join(" · "):"Ready"
        ];
      });
      return {
        title:"Registration master report",
        description:"Complete registration-control view for the Tournament Director, including status, placement, payment state, and outstanding actions.",
        defaultChart:"table",
        tableOnly:true,
        tableHeaders:["Reference","Submitted","Status","Players","Division","Assigned level","Category","Payment","Submitted amount","Action status"],
        tableRows:rows
      };
    }
    if(key==="status"){
    ...
    if(key==="actionRequired"){
      const rows=records.filter(r=>reportActionIssues(r).length).map(r=>{
        const e=recordEligibility(r);
      ...
      return {
        title:"Outstanding actions report",
        description:"The organizer work queue: registration approval, level validation, partner/category issues, missing payment proof, and payment verification.",
        defaultChart:"table",
        tableOnly:true,
        tableHeaders:["Reference","Players","Primary action","All outstanding actions","Assigned level","Division","Payment","Status"],
        tableRows:rows
      };
    }
    if(key==="dupr"){
    ...
    if(key==="paymentStatus"){
      const rows=reportGroup(records.filter(reportIsActiveRecord),reportPaymentState);
      return reportSimple("Payment status","Active registrations grouped by verified, awaiting verification, or outstanding payment state.",rows,"doughnut","Registrations");
    }
    if(key==="paymentMethod"){
      const rows=reportGroup(records.filter(reportIsActiveRecord),r=>r.payment?.method?paymentLabel(r.payment.method):"Not recorded");
      return reportSimple("Payments by method","Active registration records grouped by the payment method entered by the registrant.",rows,"pie","Registrations");
    }
    if(key==="revenueMethod"){
      const verified=records.filter(r=>reportPaymentState(r)==="Verified");
      const rows=reportGroup(verified,r=>r.payment?.method?paymentLabel(r.payment.method):"Not recorded",r=>reportPaymentVerifiedAmount(r));
      return reportSimple("Verified revenue by payment method","Verified collections grouped by the payment channel used.",rows,"hbar","Verified revenue","currency");
    }
    if(key==="paymentDivision"){
      const verified=records.filter(r=>reportPaymentState(r)==="Verified");
      const rows=reportGroup(verified,divisionName,r=>reportPaymentVerifiedAmount(r));
      return reportSimple("Verified revenue by division","Verified payment collections grouped by assigned tournament division.",rows,"hbar","Verified revenue","currency");
    }
    if(key==="financial"){
      const f=reportFinanceSnapshot(records);
      const rows=[
        {label:"Expected active fees",value:f.expected},
        {label:"Payment amounts submitted",value:f.submitted},
        {label:"Verified collected",value:f.verified},
        {label:"Awaiting verification",value:f.awaiting},
        {label:"Uncollected / no amount submitted",value:f.uncollected},
        {label:"Verified balance outstanding",value:f.verifiedOutstanding}
      ];
      return {
        title:"Financial report",
        description:"Tournament-level registration finance summary separating expected fees, submitted amounts, verified collections, and balances still requiring collection or verification.",
        rows,
        defaultChart:"bar",
        valueLabel:"Amount",
        valueFormat:"currency",
        tableHeaders:["Financial metric","Amount","Management note"],
        tableRows:[
          ["Expected active registration fees",formatMoney(f.expected),`${f.active.length} active registration${f.active.length===1?"":"s"}`],
          ["Payment amounts submitted",formatMoney(f.submitted),"Amounts entered or submitted with registration payment records"],
          ["Verified collected",formatMoney(f.verified),`${f.collectionRate}% verified collection rate`],
          ["Awaiting payment verification",formatMoney(f.awaiting),`${f.awaitingCount} registration${f.awaitingCount===1?"":"s"} requiring finance review`],
          ["Uncollected / no amount submitted",formatMoney(f.uncollected),`${f.outstandingCount} registration${f.outstandingCount===1?"":"s"} currently outstanding`],
          ["Verified balance outstanding",formatMoney(f.verifiedOutstanding),"Expected fees minus verified collections"],
          ["Potential overpayments",formatMoney(f.overpaid),"Submitted amount above expected fee, where present"],
          ["Refunds recorded",formatMoney(f.refunds),"Uses refundAmount when present in payment records"],
          ["Adjustments recorded",formatMoney(f.adjustments),"Uses adjustmentAmount when present in payment records"]
        ]
      };
    }
    if(key==="paymentReconciliation"){
      const rows=records.filter(reportIsActiveRecord).map(r=>{
        const expected=reportPaymentExpected(r);
        const submitted=reportPaymentSubmitted(r);
        const difference=submitted-expected;
        return [
          r.reference||"—",
          [fullName(r.player1),r.registrationType==="pair"?fullName(r.player2):""].filter(Boolean).join(" / ")||"—",
          divisionName(r),
          r.payment?.method?paymentLabel(r.payment.method):"—",
          formatMoney(expected),
          formatMoney(submitted),
          formatMoney(difference),
          reportPaymentState(r),
          reportPaymentProofState(r),
          r.payment?.reference||"—",
          r.payment?.senderName||"—",
          r.payment?.paymentDate||"—",
          r.payment?.verifiedByEmail||"—",
          r.payment?.verifiedAt?new Date(r.payment.verifiedAt).toLocaleString("en-PH"):"—"
        ];
      });
      return {
        title:"Payment reconciliation report",
        description:"Transaction-level finance working paper for reconciling expected fees, submitted amounts, proof, verification, reference numbers, and verifier details.",
        defaultChart:"table",
        tableOnly:true,
        tableHeaders:["Reference","Players","Division","Method","Expected","Submitted","Variance","Payment status","Proof","Payment reference","Sender","Payment date","Verified by","Verified at"],
        tableRows:rows
      };
    }
    if(key==="playersTeams"){
      const rows=records.map(r=>{
        const p1Level=reportPlayerLevel(r.verification1);
        const p2Level=r.registrationType==="pair"?reportPlayerLevel(r.verification2):"—";
        const e=recordEligibility(r);
        const mismatch=!!e.higherLevelApplied;
        return [
          r.reference||"—",
          fullName(r.player1),
          p1Level,
          r.registrationType==="pair"?fullName(r.player2):"—",
          p2Level,
          e.levelKey?levelLabel(e.levelKey):e.label||"Pending",
          mismatch?`Yes — placed in ${levelLabel(e.levelKey)}`:"No",
          e.categoryLabel||"Pending",
          divisionName(r),
          workflowStatus(r.status)
        ];
      ...
      return {
        title:"Player & team report",
        description:"Team composition, each player's assessed level, higher-tier placement when levels differ, category, division, and registration status.",
        defaultChart:"table",
        tableOnly:true,
        tableHeaders:["Reference","Player 1","Player 1 level","Player 2","Player 2 level","Assigned level","Different levels?","Category","Division","Status"],
        tableRows:rows
      };
    }
    if(key==="tournamentRoster"){
      const approved=records.filter(r=>workflowStatus(r.status)==="Approved");
      const rows=approved.map(r=>{
        ...
        return [
          ...
          r.player1?.mobile||"—",
          r.registrationType==="pair"?fullName(r.player2):"—",
          r.registrationType==="pair"?(r.player2?.mobile||"—"):"—",
          reportPaymentState(r),
          reportKnownPlayers(r).map(p=>p.verification?.jerseyName||"—").join(" / ")
        ];
      }).sort((a,b)=>String(a[1]).localeCompare(String(b[1]))||String(a[4]).localeCompare(String(b[4])));
      return {
        title:"Final tournament roster",
        description:"Approved entries only, organized for operational handoff into check-in and Match Control.",
        defaultChart:"table",
        tableOnly:true,
        tableHeaders:["Reference","Division","Level","Category","Player 1","Player 1 mobile","Player 2","Player 2 mobile","Payment","Jersey back name(s)"],
        tableRows:rows
      };
    }
    if(key==="auditTrail"){
      const rows=allActivityRows().map(row=>[
        row.at?new Date(row.at).toLocaleString("en-PH"):"—",
        row.actorName||"—",
        row.role||"—",
        row.category||"—",
        row.action||"—",
      ...
      return {
        ...
        description:"Organizer actions, access events, registration decisions, and notification events available to this admin session/device.",
        defaultChart:"table",
        tableOnly:true,
        tableHeaders:["Date / time","Actor","Role","Category","Action","Target","Details","Source","Result"],
        tableRows:rows
      };
    }
    if(key==="emailDelivery"){
      const rows=getEmailActivity().map(row=>[
        row.at?new Date(row.at).toLocaleString("en-PH"):"—",
        row.templateLabel||"Email Notification",
        row.reference||"—",
        row.recipientName||"—",
        row.recipientEmail||"—",
      ...
      return {
        title:"Email / notification report",
        description:"Notification trigger and delivery activity currently available in the portal's email activity view. This remains device/session-side until full server email-history reporting is enabled.",
        defaultChart:"table",
        tableOnly:true,
        tableHeaders:["Date / time","Template","Registration","Recipient","Email","Result","Trigger / source","Detail"],
        tableRows:rows
      };
    }
    if(key==="capacity"){
  ...
  function executiveKpiCard(label,value,help,tone="green",icon="●",drillKey="",drillLabel=""){
    const attrs=drillKey?reportDrillAttrs(drillKey,drillLabel||label):"";
    return `<article class="executive-kpi executive-kpi-${esc(tone)} ${drillKey?"report-drill-target":""}"${attrs}><div class="executive-kpi-icon" aria-hidden="true">${icon}</div><div><span>${esc(label)}</span><strong>${value}</strong><small>${esc(help)}</small></div></article>`;
  }

  function executiveCapacitySnapshot(records){
    const m=analystMetrics(records);
    const rows=[...(m.capacity?.rows||[])].filter(r=>r.capacity).sort((a,b)=>b.utilization-a.utilization||b.active-a.active);
    const hottest=rows[0]||null;
    return {metrics:m,hottest};
  }

  function executiveRingMarkup(percent,main,sub){
    const pct=Math.max(0,Math.min(100,Number(percent)||0));
    return `<div class="executive-ring" style="--ring-pct:${pct}"><div class="executive-ring-center"><strong>${main}</strong><span>${sub}</span></div></div>`;
  }

  function executivePaymentCard(records){
    const f=reportFinanceSnapshot(records);
    const rate=f.expected?Math.round(f.verified/f.expected*100):0;
    return `<article class="executive-visual-card executive-payment-card">
      <div class="executive-visual-head"><div><h3>Payment Verification</h3><p>Verified collections against expected active fees</p></div></div>
      <div class="executive-ring-layout">
        ${executiveRingMarkup(rate,formatMoney(f.verified),`of ${formatMoney(f.expected)} expected`)}
        <div class="executive-ring-legend">
          <div class="report-drill-target"${reportDrillAttrs("paymentStatus","Verified")}><i class="legend-dot verified"></i><span>Verified</span><strong>${rate}%</strong><small>${formatMoney(f.verified)}</small></div>
          <div class="report-drill-target"${reportDrillAttrs("financial","Verified balance outstanding")}><i class="legend-dot pending"></i><span>Pending / outstanding</span><strong>${Math.max(100-rate,0)}%</strong><small>${formatMoney(Math.max(f.expected-f.verified,0))}</small></div>
        </div>
      </div>
      <div class="executive-card-note"><span>▣</span><p>${f.awaitingCount?`${f.awaitingCount} payment${f.awaitingCount===1?" is":"s are"} awaiting verification. `:""}${rate}% of expected active fees are verified.</p></div>
    </article>`;
  }

  function executiveCapacityCard(records){
    const {metrics:m,hottest}=executiveCapacitySnapshot(records);
    const pct=hottest?.utilization??m.capacity?.utilization??0;
    const active=hottest?.active??m.capacity?.active??0;
    const cap=hottest?.capacity??m.capacity?.capacity??0;
    const name=hottest?.division?.name||"Configured divisions";
    return `<article class="executive-visual-card executive-capacity-card">
      <div class="executive-visual-head"><div><h3>Capacity Utilization</h3><p>Highest-utilized division in the current scope</p></div></div>
      <div class="executive-ring-layout">
        ${executiveRingMarkup(pct,`${pct}%`,cap?`${active} of ${cap} slots`:"No capacity")}
        <div class="executive-ring-legend single">
          <div class="report-drill-target"${reportDrillAttrs("capacity",name)}><i class="legend-dot verified"></i><span>Filled</span><strong>${active}</strong><small>${esc(name)}</small></div>
          <div class="report-drill-target"${reportDrillAttrs("capacity",name)}><i class="legend-dot pending"></i><span>Available</span><strong>${cap?Math.max(cap-active,0):"—"}</strong><small>Remaining team slots</small></div>
        </div>
      </div>
      <div class="executive-card-note"><span>●</span><p>${hottest?`${esc(name)} is currently the most utilized division at ${pct}% (${active}/${cap} slots).`:"Capacity will populate as configured divisions receive registrations."}</p></div>
    </article>`;
  }

  function executiveRecentRegistrations(records){
    const sorted=[...records].sort((a,b)=>new Date(b.submittedAt||0)-new Date(a.submittedAt||0)).slice(0,6);
    const body=sorted.length?sorted.map(r=>{
      ...
      const level=e.levelKey?levelLabel(e.levelKey):e.label||"Pending";
      const category=e.categoryLabel||"Pending";
      const status=workflowStatus(r.status);
      const payment=reportPaymentState(r);
      const amount=reportPaymentSubmitted(r);
      const date=r.submittedAt?new Date(r.submittedAt).toLocaleDateString("en-PH",{month:"short",day:"numeric",year:"numeric"}):"—";
      const statusClass=status.toLowerCase().replace(/[^a-z0-9]+/g,"-");
      const payClass=payment.toLowerCase().replace(/[^a-z0-9]+/g,"-");
      return `<tr><td>${esc(date)}</td><td><strong>${esc(players)}</strong><small>${esc(r.reference||"")}</small></td><td>${esc(division)}</td><td>${esc(level)}</td><td>${esc(category)}</td><td><span class="executive-pill status-${esc(statusClass)}">${esc(status)}</span></td><td><span class="executive-pill payment-${esc(payClass)}">${esc(payment)}</span></td><td>${formatMoney(amount)}</td></tr>`;
    }).join(""):`<tr><td colspan="8" class="executive-empty-row">No registrations match the current report filters.</td></tr>`;
    return `<section class="executive-recent-card">
      <div class="executive-section-head"><div><h3>Recent Registrations</h3><p>Latest registration activity in the selected scope</p></div><button class="executive-link-button" type="button" data-director-report="registrationSummary">View all registrations →</button></div>
      <div class="executive-table-scroll"><table class="executive-table"><thead><tr><th>Date</th><th>Player / Team</th><th>Division</th><th>Level</th><th>Category</th><th>Status</th><th>Payment</th><th>Amount</th></tr></thead><tbody>${body}</tbody></table></div>
    </section>`;
  }

  function executiveAnalystPanel(records){
    const m=analystMetrics(records);
    const cap=[...(m.capacity?.rows||[])].filter(r=>r.capacity).sort((a,b)=>b.utilization-a.utilization)[0]||null;
    const interest=records.length?`${m.approved} approved (${m.approvalRate}%) and ${m.underReview} still under review (${m.reviewRate}%).`:"No registrations are currently in scope.";
    const readiness=m.k.needsReview?`${m.k.needsReview} registration${m.k.needsReview===1?"":"s"} require eligibility, approval, or payment action.`:"No registrations are currently flagged for action.";
    const capacity=cap?`${cap.division.name} has the highest utilization at ${cap.utilization}% (${cap.active}/${cap.capacity} slots).`:"No configured division capacity is available yet.";
    const finance=can("reports.financial")?`${formatMoney(m.finance.verifiedOutstanding)} remains unverified or outstanding against expected active fees.`:`DUPR coverage is ${m.k.duprCoverage}% across ${m.players.length} known player${m.players.length===1?"":"s"}.`;
    return `<aside class="executive-analyst-card">
      <div class="executive-section-head compact"><div><h3>Management Insights</h3><p>What requires management attention now</p></div></div>
      <div class="executive-insight-list">
        <article class="insight-green"><i>↗</i><div><strong>Registration position</strong><p>${esc(interest)}</p></div></article>
        <article class="insight-yellow"><i>◷</i><div><strong>Focus on operational readiness</strong><p>${esc(readiness)}</p></div></article>
        <article class="insight-blue"><i>●</i><div><strong>Monitor division capacity</strong><p>${esc(capacity)}</p></div></article>
        <article class="insight-coral"><i>▣</i><div><strong>${can("reports.financial")?"Continue payment follow-up":"Monitor data quality"}</strong><p>${esc(finance)}</p></div></article>
      </div>
    </aside>`;
  }

  function executiveOverviewMarkup(records){
    const statusDef=reportDefinition("status",records);
    const divisionDef=reportDefinition("division",records);
    const m=analystMetrics(records);
    return `<div class="executive-dashboard-overview">
      <div class="executive-visual-grid">
        <article class="executive-visual-card">
          <div class="executive-visual-head"><div><h3>Registration Status Mix</h3><p>Distribution of all registrations by status</p></div></div>
          <div class="executive-chart-body executive-status-chart">${reportChartMarkupForType(statusDef,"doughnut","status")}</div>
          <div class="executive-card-note"><span>◉</span><p>${esc(analystInterpretation("status",records))}</p></div>
        </article>
        <article class="executive-visual-card">
          <div class="executive-visual-head"><div><h3>Division Demand</h3><p>Registration demand by assigned division</p></div></div>
          <div class="executive-chart-body executive-division-chart">${reportChartMarkupForType(divisionDef,"hbar","division")}</div>
          <div class="executive-card-note"><span>▥</span><p>${esc(analystInterpretation("division",records))}</p></div>
        </article>
        ${can("reports.financial")?executivePaymentCard(records):analystVisualCard("verification",records,"doughnut",{kicker:"Readiness",title:"Validation Workload"})}
        ${executiveCapacityCard(records)}
      </div>
      <div class="executive-lower-grid">
        ${executiveRecentRegistrations(records)}
        ${executiveAnalystPanel(records)}
      </div>
    </div>`;
  }
  ...
  function reportWrapSvgLabel(label,maxChars=18,maxLines=3){
  ...
  function reportBarChart(rows,format="number",horizontal=false,interactionKey=""){
    ...
    if(horizontal){
      ...
      const bars=data.map((r,i)=>{
        ...
        const color=reportElementColor(interactionKey,r,i);
        const attrs=interactionKey?reportDrillAttrs(interactionKey,r.label):"";
        return `<g class="${interactionKey?"report-drill-target":""}"${attrs}>
          <title>${esc(r.label)}: ${esc(reportFormatValue(r.value,format))}${interactionKey?" · Click to view breakdown":""}</title>
          <text x="${left-12}" y="${y+17}" text-anchor="end" class="bi-svg-label">${esc(r.label)}</text>
      ...
      return `<svg class="bi-svg-chart bi-svg-hbar ${interactionKey?"is-drillable":""}" viewBox="0 0 ${w} ${h}" role="img" aria-label="Horizontal bar chart">${bars}</svg>`;
    }
    const wrapAxisLabels=data.length<=8;
    const w=960,h=420,left=56,right=22,top=26,bottom=wrapAxisLabels?112:88;
    ...
    const bars=data.map((r,i)=>{
      ...
      let labelMarkup="";
      if(wrapAxisLabels){
        const maxChars=Math.max(12,Math.min(20,Math.floor(slot/7.4)));
        const lines=reportWrapSvgLabel(rawLabel,maxChars,3);
        labelMarkup=`<text x="${x+barW/2}" y="${top+plotH+22}" text-anchor="middle" class="bi-svg-label">${lines.map((line,lineIndex)=>`<tspan x="${x+barW/2}" dy="${lineIndex?13:0}">${esc(line)}</tspan>`).join("")}</text>`;
      }else{
        const label=rawLabel.length>17?rawLabel.slice(0,16)+"…":rawLabel;
        labelMarkup=`<text x="${x+barW/2}" y="${top+plotH+22}" text-anchor="end" transform="rotate(-38 ${x+barW/2} ${top+plotH+22})" class="bi-svg-label">${esc(label)}</text>`;
      }
      const attrs=interactionKey?reportDrillAttrs(interactionKey,r.label):"";
      return `<g class="${interactionKey?"report-drill-target":""}"${attrs}>
        <title>${esc(rawLabel)}: ${esc(reportFormatValue(v,format))}${interactionKey?" · Click to view breakdown":""}</title>
        <rect x="${x}" y="${y}" width="${barW}" height="${Math.max(bh,2)}" rx="7" fill="${color}"></rect>
    ...
    return `<svg class="bi-svg-chart ${interactionKey?"is-drillable":""}" viewBox="0 0 ${w} ${h}" role="img" aria-label="Bar chart">${grid}${bars}</svg>`;
  }

  function reportLineChart(rows,format="number",area=false,interactionKey=""){
    if(!rows.length)return reportEmptyChart();
    ...
    const dots=points.map((p,i)=>{
      const attrs=interactionKey?reportDrillAttrs(interactionKey,p.row.rawLabel||p.row.label):"";
      return `<circle class="${interactionKey?"report-drill-target":""}"${attrs} cx="${p.x}" cy="${p.y}" r="${interactionKey?7:4.5}" fill="${reportElementColor(interactionKey,p.row,i)}"><title>${esc(p.row.label)}: ${esc(reportFormatValue(p.row.value,format))}${interactionKey?" · Click to view breakdown":""}</title></circle>`;
    }).join("");
    return `<svg class="bi-svg-chart ${interactionKey?"is-drillable":""}" viewBox="0 0 ${w} ${h}" role="img" aria-label="${area?"Area":"Line"} chart">
  ...
  function reportPieChart(rows,format="number",doughnut=false,interactionKey=""){
    ...
    const stops=data.map((r,i)=>{
      ...
      return `${reportElementColor(interactionKey,r,i)} ${start.toFixed(2)}% ${running.toFixed(2)}%`;
    }).join(",");
    const pieItems=encodeURIComponent(JSON.stringify(data.map(r=>({label:r.rawLabel||r.label,value:Number(r.value)||0}))));
    const pieAttrs=interactionKey?` data-report-pie-key="${esc(interactionKey)}" data-report-pie-items="${pieItems}" role="button" tabindex="0" aria-label="Click a chart segment to view its breakdown"`:"";
    return `<div class="bi-pie-layout ${interactionKey?"is-drillable":""}">
      <div class="bi-pie ${doughnut?"bi-doughnut":""} ${interactionKey?"report-pie-drill-target":""}"${pieAttrs} style="background:conic-gradient(${stops})">
      ...
      <div class="bi-legend">${data.map((r,i)=>{
        ...
        const attrs=interactionKey?reportDrillAttrs(interactionKey,r.rawLabel||r.label):"";
        return `<div class="${interactionKey?"report-drill-target":""}"${attrs}><i style="background:${reportElementColor(interactionKey,r,i)}"></i><span>${esc(r.label)}</span><strong>${esc(reportFormatValue(r.value,format))}</strong><small>${pct}%</small></div>`;
      }).join("")}</div>
    </div>`;
  }

  function reportScatterChart(points,interactionKey=""){
    ...
    const dots=points.map((p,i)=>{
      const c=reportElementColor(interactionKey,{label:p.label},i);
      const attrs=interactionKey?reportDrillAttrs(interactionKey,p.label):"";
      return `<circle class="${interactionKey?"report-drill-target":""}"${attrs} cx="${xPos(p.x)}" cy="${yPos(p.y)}" r="${interactionKey?8:6}" fill="${c}" opacity=".86"><title>${esc(p.label)} · Age ${p.x} · DUPR ${p.y.toFixed(2)} · ${esc(p.club)}${interactionKey?" · Click to view player details":""}</title></circle>`;
    }).join("");
  ...
  function reportChartMarkupForType(def,requestedType="auto",interactionKey=""){
    ...
    if(type==="scatter")return reportScatterChart(def.points||[],interactionKey);
    const rows=def.rows||[];
    if(type==="bar")return reportBarChart(rows,def.valueFormat,false,interactionKey);
    if(type==="hbar")return reportBarChart(rows,def.valueFormat,true,interactionKey);
    if(type==="line")return reportLineChart(rows,def.valueFormat,false,interactionKey);
    if(type==="area")return reportLineChart(rows,def.valueFormat,true,interactionKey);
    if(type==="pie")return reportPieChart(rows,def.valueFormat,false,interactionKey);
    if(type==="doughnut")return reportPieChart(rows,def.valueFormat,true,interactionKey);
    return reportBarChart(rows,def.valueFormat,false,interactionKey);
  }

  function reportChartMarkup(def){
    return reportChartMarkupForType(def,reportChartType,reportKey);
  }
  ...
  function reportDrilldownScopeRecords(){
    const all=getRecords().filter(r=>!r.deleted);
    return reportViewMode==="summary"?all:reportFilteredRecords();
  }

  function reportDrilldownDivisionName(record){
    return (record?.divisions||[]).map(id=>getDivision(id)?.name||id).filter(Boolean).join(" | ")||"Pending";
  }

  function reportDrilldownLevelName(record){
    const e=recordEligibility(record);
    return e.levelKey?levelLabel(e.levelKey):(e.label||"Pending");
  }

  function reportDrilldownCategoryName(record){
    return recordEligibility(record).categoryLabel||"Pending";
  }

  function reportDrilldownPlayers(record){
    return [fullName(record?.player1),record?.registrationType==="pair"?fullName(record?.player2):""].filter(Boolean).join(" + ")||"—";
  }

  function reportDrilldownAction(record){
    const issues=reportActionIssues(record);
    return issues.length?issues.join(" · "):"Ready";
  }

  function reportDrilldownPaymentMethod(record){
    return record?.payment?.method?paymentLabel(record.payment.method):"—";
  }

  function reportDrilldownPaymentReference(record){
    return record?.payment?.reference||record?.payment?.paymentReference||"—";
  }

  function reportDrilldownRecordRow(record,selectedAmount=null,reason=""){
    const expected=reportPaymentExpected(record);
    const submitted=reportPaymentSubmitted(record);
    const verified=reportPaymentVerifiedAmount(record);
    return {
      reference:record.reference||"",
      cells:[
        record.reference||"—",
        reportDrilldownPlayers(record),
        reportDrilldownDivisionName(record),
        formatMoney(expected),
        formatMoney(submitted),
        formatMoney(verified),
        selectedAmount==null?"—":formatMoney(selectedAmount),
        reportPaymentState(record),
        reportDrilldownPaymentMethod(record),
        reportDrilldownPaymentReference(record),
        reportPaymentProofState(record),
        reason||reportDrilldownAction(record)
      ]
    };
  }

  function reportDrilldownStandardRecordRow(record){
    return {
      reference:record.reference||"",
      cells:[
        record.reference||"—",
        reportDrilldownPlayers(record),
        reportDrilldownDivisionName(record),
        reportDrilldownLevelName(record),
        reportDrilldownCategoryName(record),
        reportPaymentState(record),
        workflowStatus(record.status),
        record.submittedAt?new Date(record.submittedAt).toLocaleString("en-PH"):"—",
        reportDrilldownAction(record)
      ]
    };
  }

  function reportDrilldownFinancial(label,records){
    const active=records.filter(reportIsActiveRecord);
    ...
    active.forEach(record=>{
      ...
      const submitted=reportPaymentSubmitted(record);
      const verified=reportPaymentVerifiedAmount(record);
      const state=reportPaymentState(record);
      let contribution=0;
      let reason="";
      if(normalized==="Expected active fees"){
        contribution=expected;
        reason="Expected registration fee";
      }else if(normalized==="Payment amounts submitted"){
        contribution=submitted;
        reason=submitted>expected?"Submitted amount exceeds expected fee":"Payment amount submitted";
      }else if(normalized==="Verified collected"){
        contribution=verified;
        reason=verified>0?"Verified collection":"";
      }else if(normalized==="Awaiting verification"){
        contribution=state==="Awaiting verification"?submitted:0;
        reason=contribution>0?"Payment received; verification pending":"";
      }else if(normalized==="Uncollected / no amount submitted"){
        contribution=Math.max(expected-submitted,0);
        reason=contribution>0?(submitted>0?"Partial payment / remaining balance not submitted":"No payment amount submitted"):"";
      }else if(normalized==="Verified balance outstanding"){
        contribution=Math.max(expected-verified,0);
        reason=contribution>0?(state==="Awaiting verification"?"Received but not yet verified":state==="Outstanding"?"Payment not yet received":"Balance remaining against verified collections"):"";
      }
      if(contribution>0)selected.push({record,contribution,reason});
    });
    const total=selected.reduce((s,x)=>s+x.contribution,0);
    return {
      title:normalized||"Financial breakdown",
      kicker:"Financial drill-down",
      description:"Every row below contributes directly to the selected chart amount. Use View Registration to open the underlying account and take action.",
      totalLabel:"Selected total",
      totalValue:formatMoney(total),
      countLabel:`${selected.length} registration${selected.length===1?"":"s"}`,
      reconcile:`Breakdown total: ${formatMoney(total)}`,
      headers:["Reference","Players","Division","Expected","Submitted","Verified","Selected amount","Payment state","Method","Payment reference","Proof","Reason / next action"],
      rows:selected.map(x=>reportDrilldownRecordRow(x.record,x.contribution,x.reason))
    };
  }

  function reportDrilldownPlayerRows(key,label,records){
    const players=reportPlayerRows(records);
    const normalized=String(label||"");
    let selected=players;
    if(key==="dupr")selected=players.filter(p=>{
      const bucket=p.hasDupr==="Yes"?"Has DUPR":p.hasDupr==="No"?"No DUPR":"Not provided";
      return normalized==="__all__"||bucket===normalized;
    });
    else if(key==="ratingBands")selected=players.filter(p=>normalized==="__all__"||reportRatingBand(p)===normalized);
    else if(key==="club")selected=players.filter(p=>normalized==="__all__"||p.club===normalized);
    else if(key==="gender")selected=players.filter(p=>normalized==="__all__"||p.gender===normalized);
    else if(key==="ageGroups")selected=players.filter(p=>normalized==="__all__"||reportAgeBand(p.age)===normalized);
    else if(key==="ageVsDupr")selected=players.filter(p=>normalized==="__all__"||p.name===normalized);
    return {
      title:normalized==="__all__"?"Player breakdown":normalized,
      kicker:"Player drill-down",
      description:"Players behind the selected chart element. Open the registration to review the complete account.",
      totalLabel:"Players",
      totalValue:selected.length.toLocaleString("en-PH"),
      countLabel:`${new Set(selected.map(p=>p.record?.reference).filter(Boolean)).size} registration${new Set(selected.map(p=>p.record?.reference).filter(Boolean)).size===1?"":"s"}`,
      headers:["Player","Registration","Division","Level","DUPR","Club","Age","Gender"],
      rows:selected.map(p=>({
        reference:p.record?.reference||"",
        cells:[
          p.name||"—",
          p.record?.reference||"—",
          reportDrilldownDivisionName(p.record),
          p.level||"Pending",
  ...
  function reportDrilldownData(key,label,records){
    const normalized=String(label||"__all__");
    if(key==="financial")return reportDrilldownFinancial(normalized,records);
    if(["dupr","ratingBands","club","gender","ageGroups","ageVsDupr"].includes(key)){
      return reportDrilldownPlayerRows(key,normalized,records);
    }

    let selected=[...records];
    let title=normalized==="__all__"?"Registration breakdown":normalized;
    let description="Registrations behind the selected chart element.";
    let extraSummary="";
    ...
    if(key==="registrationSummary"){
      ...
      selected=records.filter(r=>reportActionIssues(r).length);
      title="Registrations needing attention";
      description="Registrations with unresolved eligibility, approval, or payment follow-up.";
    }else if(key==="status"){
      selected=records.filter(r=>normalized==="__all__"||workflowStatus(r.status)===normalized);
      title=normalized==="__all__"?"Registration status breakdown":`${normalized} registrations`;
    }else if(key==="division"){
      selected=records.filter(r=>normalized==="__all__"||reportDrilldownDivisionName(r)===normalized);
      title=normalized==="__all__"?"Division demand":normalized;
    }else if(key==="level"){
      selected=records.filter(r=>normalized==="__all__"||reportDrilldownLevelName(r)===normalized);
      title=normalized==="__all__"?"Level breakdown":normalized;
    }else if(key==="category"){
      selected=records.filter(r=>normalized==="__all__"||reportDrilldownCategoryName(r)===normalized);
      title=normalized==="__all__"?"Category breakdown":normalized;
    }else if(key==="trend"){
      selected=records.filter(r=>{
        ...
        return normalized==="__all__"||raw===normalized||new Date(`${raw}T00:00:00`).toLocaleDateString("en-PH",{month:"short",day:"numeric"})===normalized;
      });
      title=normalized==="__all__"?"Registration trend":`Registrations on ${normalized}`;
    }else if(key==="verification"){
      selected=records.filter(r=>normalized==="__all__"||reportVerificationBucket(r)===normalized);
      title=normalized==="__all__"?"Eligibility workload":normalized;
    }else if(key==="paymentStatus"){
      selected=records.filter(r=>reportIsActiveRecord(r)&&(normalized==="__all__"||reportPaymentState(r)===normalized));
      title=normalized==="__all__"?"Payment status":normalized;
    }else if(key==="paymentMethod"){
      selected=records.filter(r=>reportIsActiveRecord(r)&&(normalized==="__all__"||reportDrilldownPaymentMethod(r)===normalized));
      title=normalized==="__all__"?"Payment method":normalized;
    }else if(key==="revenueMethod"){
      selected=records.filter(r=>reportPaymentState(r)==="Verified"&&(normalized==="__all__"||reportDrilldownPaymentMethod(r)===normalized));
      title=normalized==="__all__"?"Verified revenue by method":normalized;
    }else if(key==="paymentDivision"){
      selected=records.filter(r=>reportPaymentState(r)==="Verified"&&(normalized==="__all__"||reportDrilldownDivisionName(r)===normalized));
      title=normalized==="__all__"?"Verified revenue by division":normalized;
    }else if(key==="capacity"){
      const division=(config.divisions||[]).find(d=>d.name===normalized||d.id===normalized);
      selected=records.filter(r=>{
        if(!reportIsActiveRecord(r))return false;
        if(normalized==="__all__")return !!(r.divisions||[]).length;
        return division?(r.divisions||[]).includes(division.id):reportDrilldownDivisionName(r)===normalized;
      });
      if(division){
        const cap=Number(division.capacity)||0;
        const util=cap?Math.round(selected.length/cap*100):0;
        extraSummary=cap?`${selected.length} of ${cap} team slots · ${util}% utilized`:`${selected.length} assigned registrations`;
      }
      title=normalized==="__all__"?"Capacity utilization":normalized;
      description="Active registrations occupying the selected division capacity.";
    }

    return {
      title,
      kicker:"Report drill-down",
      description,
      totalLabel:"Registrations",
      totalValue:selected.length.toLocaleString("en-PH"),
      countLabel:extraSummary||`${selected.length} account${selected.length===1?"":"s"}`,
      headers:["Reference","Players","Division","Level","Category","Payment","Registration status","Submitted","Action / follow-up"],
      rows:selected.map(reportDrilldownStandardRecordRow)
    };
  }

  function ensureReportDrilldownModal(){
    let modal=$("#reportDrilldownModal");
    if(modal)return modal;
    modal=document.createElement("dialog");
    modal.id="reportDrilldownModal";
    modal.className="report-drilldown-dialog";
    modal.innerHTML=`<div class="report-drilldown-shell">
      <header class="report-drilldown-head">
        <div><span class="eyebrow" id="reportDrilldownKicker">Report drill-down</span><h2 id="reportDrilldownTitle">Breakdown</h2><p id="reportDrilldownDescription"></p></div>
        <button class="report-drilldown-close" id="reportDrilldownClose" type="button" aria-label="Close breakdown">×</button>
      </header>
      <div class="report-drilldown-scroll">
        <section class="report-drilldown-summary" id="reportDrilldownSummary"></section>
        <div class="report-drilldown-table-wrap" id="reportDrilldownTable"></div>
      </div>
      <footer class="report-drilldown-actions">
        <button class="button button-ghost" id="reportDrilldownExport" type="button">Export This Breakdown</button>
        <button class="button button-primary" id="reportDrilldownDone" type="button">Close</button>
      </footer>
    </div>`;
    document.body.appendChild(modal);
    $("#reportDrilldownClose").onclick=()=>modal.close();
    $("#reportDrilldownDone").onclick=()=>modal.close();
    modal.addEventListener("click",event=>{if(event.target===modal)modal.close()});
    $("#reportDrilldownExport").onclick=exportReportDrilldownCsv;
    return modal;
  }

  function renderReportDrilldown(data){
    const modal=ensureReportDrilldownModal();
    reportDrilldownState=data;
    $("#reportDrilldownKicker").textContent=data.kicker||"Report drill-down";
    $("#reportDrilldownTitle").textContent=data.title||"Breakdown";
    $("#reportDrilldownDescription").textContent=data.description||"";
    $("#reportDrilldownSummary").innerHTML=`
      <div><span>${esc(data.totalLabel||"Total")}</span><strong>${esc(data.totalValue||"0")}</strong></div>
      <div><span>Records in breakdown</span><strong>${Number(data.rows?.length||0).toLocaleString("en-PH")}</strong></div>
      <div class="report-drilldown-summary-note"><span>Scope</span><strong>${esc(data.countLabel||"Current report scope")}</strong></div>
      ${data.reconcile?`<div class="report-drilldown-reconcile"><span>✓</span><strong>${esc(data.reconcile)}</strong><small>Rows reconcile to the selected chart value.</small></div>`:""}
    `;
    const headers=data.headers||[];
    const rows=data.rows||[];
    $("#reportDrilldownTable").innerHTML=rows.length?`
      <table class="report-drilldown-table">
        <thead><tr>${headers.map(h=>`<th>${esc(h)}</th>`).join("")}<th>Account</th></tr></thead>
        <tbody>${rows.map(row=>`<tr>${row.cells.map((cell,i)=>`<td class="${i===0?"is-primary":""}">${esc(cell??"—")}</td>`).join("")}<td>${row.reference?`<button class="report-drilldown-view" type="button" data-report-drill-view="${esc(row.reference)}">View Registration</button>`:"—"}</td></tr>`).join("")}</tbody>
      </table>`:`<div class="report-drilldown-empty"><strong>No matching records</strong><p>No registrations currently contribute to this chart element in the selected report scope.</p></div>`;
    $$("[data-report-drill-view]").forEach(btn=>btn.onclick=()=>{
      const ref=btn.dataset.reportDrillView;
      modal.close();
  ...
  function openReportDrilldown(key,label){
    const records=reportDrilldownScopeRecords();
    renderReportDrilldown(reportDrilldownData(key,label,records));
  }

  function exportReportDrilldownCsv(){
    const data=reportDrilldownState;
    if(!data?.rows?.length){showToast("There is no breakdown data to export");return}
    const headers=[...(data.headers||[]),"Registration"];
    const rows=data.rows.map(row=>[...row.cells,row.reference||""]);
    const csv=[headers,...rows].map(row=>row.map(csvCell).join(",")).join("\n");
    download(csv,`animo-${reportFileSlug(data.title||"report-breakdown")}-breakdown.csv`,"text/csv;charset=utf-8");
    showToast("Breakdown CSV exported");
  }

  function handleReportPieDrillClick(target,event){
    const key=target.dataset.reportPieKey;
    let items=[];
    try{items=JSON.parse(decodeURIComponent(target.dataset.reportPieItems||"[]"))}catch(e){}
    if(!key||!items.length)return;
    const rect=target.getBoundingClientRect();
    if(!rect.width||!rect.height)return;
    const x=(event.clientX??(rect.left+rect.width/2))-rect.left-rect.width/2;
    const y=(event.clientY??rect.top)-rect.top-rect.height/2;
    let degrees=(Math.atan2(y,x)*180/Math.PI+90+360)%360;
    const total=items.reduce((s,item)=>s+Math.max(0,Number(item.value)||0),0);
    if(total<=0)return;
    const point=degrees/360*total;
    let running=0;
    let chosen=items[items.length-1];
    for(const item of items){
      running+=Math.max(0,Number(item.value)||0);
      if(point<=running){chosen=item;break}
    }
    openReportDrilldown(key,chosen.label);
  }

  function bindReportDrilldowns(){
    $$("[data-report-drill-key]").forEach(target=>{
      const activate=event=>{
        event.preventDefault();
        event.stopPropagation();
        openReportDrilldown(target.dataset.reportDrillKey,target.dataset.reportDrillLabel||"__all__");
      };
    ...
    $$("[data-report-pie-key]").forEach(target=>{
      target.onclick=event=>{
        event.preventDefault();
        event.stopPropagation();
        handleReportPieDrillClick(target,event);
      };
      target.onkeydown=event=>{
        if(event.key!=="Enter"&&event.key!==" ")return;
        event.preventDefault();
        const items=JSON.parse(decodeURIComponent(target.dataset.reportPieItems||"[]"));
        if(items[0])openReportDrilldown(target.dataset.reportPieKey,items[0].label);
      };
    });
  }
  ...
  function defaultDashboardWidgets(){
    return [
      ...
      {id:"dash-financial",reportKey:"financial",chartType:"bar",width:8,height:"standard",title:"Financial Position"},
      {id:"dash-payment-status",reportKey:"paymentStatus",chartType:"doughnut",width:4,height:"standard",title:"Payment Status"},
  ...
  function dashboardWidgetVisual(widget,records){
    ...
    return reportChartMarkupForType(def,type,widget.reportKey);
  }
  ...
  function directorCapacitySnapshot(records){
    const enabled=(config.divisions||[]).filter(d=>d.enabled);
    return enabled.map(d=>{
      const active=records.filter(r=>reportIsActiveRecord(r)&&(r.divisions||[]).includes(d.id)).length;
      const approved=records.filter(r=>workflowStatus(r.status)==="Approved"&&(r.divisions||[]).includes(d.id)).length;
      const capacity=Number(d.capacity)||0;
      return {division:d,active,approved,capacity,utilization:capacity?Math.round(active/capacity*100):0};
    });
  }

  function directorExecutiveSummaryMarkup(records){
    const k=reportKpis(records);
    const f=reportFinanceSnapshot(records);
    const capacity=directorCapacitySnapshot(records);
    const ranked=capacity.filter(x=>x.capacity).sort((a,b)=>b.utilization-a.utilization);
    const fullest=ranked[0];
    const least=ranked.length?ranked[ranked.length-1]:null;
    const pending=records.filter(r=>workflowStatus(r.status)==="Under Review").length;
    const totalCapacity=capacity.reduce((sum,x)=>sum+x.capacity,0);
    return `<section class="director-summary-section">
      <div class="director-summary-head">
        <div><span class="eyebrow">Tournament Director</span><h3>Executive Tournament Summary</h3><p>The operational picture that should drive registration, finance, and tournament-readiness decisions.</p></div>
        <button class="button button-ghost" type="button" data-director-report="executiveSummary">Open full summary</button>
      </div>
      <div class="director-summary-grid">
        <article><span>Registration requests</span><strong>${k.registrations.toLocaleString("en-PH")}</strong><small>${k.approved.toLocaleString("en-PH")} approved · ${pending.toLocaleString("en-PH")} under review</small></article>
        <article><span>Approved teams</span><strong>${k.approved.toLocaleString("en-PH")}${totalCapacity?` / ${totalCapacity.toLocaleString("en-PH")}`:""}</strong><small>${totalCapacity?"Configured tournament team capacity":"Approved registration count"}</small></article>
        <article class="${k.needsReview?"director-summary-warning":""}"><span>Needs attention</span><strong>${k.needsReview.toLocaleString("en-PH")}</strong><small>Eligibility, registration, or payment actions outstanding</small></article>
        ${can("reports.financial")?`<article><span>Verified collected</span><strong>${formatMoney(f.verified)}</strong><small>${f.collectionRate}% of ${formatMoney(f.expected)} expected active fees</small></article>`:""}
        ${can("reports.financial")?`<article class="${f.awaitingCount?"director-summary-warning":""}"><span>Awaiting verification</span><strong>${formatMoney(f.awaiting)}</strong><small>${f.awaitingCount} payment${f.awaitingCount===1?"":"s"} pending finance review</small></article>`:""}
        ${can("reports.financial")?`<article><span>Verified balance</span><strong>${formatMoney(f.verifiedOutstanding)}</strong><small>Expected active fees less verified collections</small></article>`:""}
        <article><span>Most filled division</span><strong>${esc(fullest?.division?.name||"No data yet")}</strong><small>${fullest?`${fullest.active}/${fullest.capacity} active slots · ${fullest.utilization}%`:"Capacity will populate as registrations arrive."}</small></article>
        <article><span>Least filled division</span><strong>${esc(least?.division?.name||"No data yet")}</strong><small>${least?`${least.active}/${least.capacity} active slots · ${least.utilization}%`:"Capacity will populate as registrations arrive."}</small></article>
      </div>
    </section>`;
  }

  function directorReportCenterMarkup(records){
    const available=new Set(reportDefinitions().map(([key])=>key));
    const cards=[
      ["financial","Financial Report","Expected fees, verified collections, balances, refunds/adjustments when recorded."],
      ["registrationSummary","Registration Report","Master registration control table with status, placement, payment, and action state."],
      ["capacity","Division & Capacity","Current demand, team caps, remaining slots, and utilization."],
      ["playersTeams","Players & Teams","Player levels, pair composition, and higher-tier placement when levels differ."],
      ["paymentReconciliation","Payment Reconciliation","Transaction-level finance working paper for proof and verification."],
      ["actionRequired","Outstanding Actions","Everything the organizing team still needs to resolve."],
      ["tournamentRoster","Tournament Roster","Approved entries ready for check-in and Match Control handoff."],
      ["auditTrail","Activity / Audit","Who changed what, when, and from which admin workflow."],
      ["emailDelivery","Email / Notifications","Notification trigger and delivery activity currently available to the portal."]
    ].filter(([key])=>available.has(key));
    return `<section class="director-report-center">
      <div class="director-report-center-head"><div><span class="eyebrow">Director report center</span><h3>Operational Reports</h3><p>These are the reports a Tournament Director is most likely to need before, during, and after registration.</p></div></div>
      <div class="director-report-card-grid">
        ${cards.map(([key,title,description])=>{
          const def=reportDefinition(key,records);
          const count=def.tableRows?.length||def.rows?.length||0;
          return `<button class="director-report-card" type="button" data-director-report="${esc(key)}"><span>Open report</span><strong>${esc(title)}</strong><small>${esc(description)}</small><em>${count.toLocaleString("en-PH")} ${def.tableOnly?"row":"item"}${count===1?"":"s"} in current scope</em></button>`;
        }).join("")}
      </div>
    </section>`;
  }

  function openDirectorReport(key){
    if(!reportDefinitions().some(([candidate])=>candidate===key))return;
    reportKey=key;
    reportChartType="auto";
    renderAdmin();
    setTimeout(()=>{
      const explorer=$(".bi-explorer-details");
      if(explorer){explorer.open=true;explorer.scrollIntoView({behavior:"smooth",block:"start"})}
    },0);
  }
  ...
  // Summary + full analytical report with explanations.
  // =========================================================
  function analystPercent(part,total){
    return total?Math.round(Number(part||0)/Number(total||1)*100):0;
  }

  function analystCapacityOverview(records){
    const rows=directorCapacitySnapshot(records);
    const capacity=rows.reduce((sum,row)=>sum+Number(row.capacity||0),0);
    const active=rows.reduce((sum,row)=>sum+Number(row.active||0),0);
    const approved=rows.reduce((sum,row)=>sum+Number(row.approved||0),0);
    return {
      rows,capacity,active,approved,
      utilization:capacity?Math.round(active/capacity*100):0,
      approvedUtilization:capacity?Math.round(approved/capacity*100):0
    };
  }

  function analystMetrics(records){
    const k=reportKpis(records);
    const finance=reportFinanceSnapshot(records);
    const players=reportPlayerRows(records);
    const capacity=analystCapacityOverview(records);
    const underReview=records.filter(r=>workflowStatus(r.status)==="Under Review").length;
    const approved=records.filter(r=>workflowStatus(r.status)==="Approved").length;
    const declined=records.filter(r=>workflowStatus(r.status)==="Declined").length;
    const withdrawn=records.filter(r=>workflowStatus(r.status)==="Withdrawn").length;
    const active=records.filter(reportIsActiveRecord).length;
    const mixedLevel=records.filter(r=>!!recordEligibility(r).higherLevelApplied).length;
    const paymentAction=records.filter(r=>reportIsActiveRecord(r)&&["Awaiting verification","Outstanding"].includes(reportPaymentState(r))).length;
    const eligibilityAction=records.filter(r=>{
      const e=recordEligibility(r);
      return e.manualReview||e.classificationPending||e.categoryPending||e.partnerPending;
    }).length;
    const noDupr=players.filter(p=>p.hasDupr==="No").length;
    const missingDupr=players.filter(p=>p.hasDupr!=="Yes").length;
    const jerseyComplete=players.filter(p=>String(p.verification?.jerseyName||"").trim()&&String(p.verification?.shirtSize||"").trim()).length;
    const trend=reportTrend(records);
    const registrationDays=new Set(records.map(r=>r.submittedAt?new Date(r.submittedAt).toISOString().slice(0,10):null).filter(Boolean)).size;
    const avgPerDay=registrationDays?records.length/registrationDays:0;
    return {
      k,finance,players,capacity,underReview,approved,declined,withdrawn,active,mixedLevel,paymentAction,eligibilityAction,noDupr,missingDupr,
      jerseyComplete,jerseyCompleteness:players.length?Math.round(jerseyComplete/players.length*100):0,
      approvalRate:records.length?Math.round(approved/records.length*100):0,
      reviewRate:records.length?Math.round(underReview/records.length*100):0,
      avgPerDay,trend
    };
  }

  function analystPriorityItems(records){
    const m=analystMetrics(records);
    const capacityRows=[...m.capacity.rows].filter(r=>r.capacity).sort((a,b)=>b.utilization-a.utilization);
    const hottest=capacityRows[0]||null;
    const emptiest=capacityRows.length?capacityRows[capacityRows.length-1]:null;
    const items=[];
    if(m.eligibilityAction){
      items.push({tone:"warning",label:"Eligibility",title:`${m.eligibilityAction} registration${m.eligibilityAction===1?"":"s"} need level or category review`,detail:"Clear eligibility checks before approving entries so brackets reflect the correct competitive tier."});
    }
    if(m.paymentAction){
      items.push({tone:"warning",label:"Finance",title:`${m.paymentAction} active registration${m.paymentAction===1?"":"s"} still need payment action`,detail:`Verified collections are ${m.finance.collectionRate}% of expected active fees. Prioritize missing proof and unverified payments.`});
    }
    if(hottest&&hottest.utilization>=80){
      items.push({tone:"critical",label:"Capacity",title:`${hottest.division.name} is ${hottest.utilization}% full`,detail:`Only ${Math.max(hottest.capacity-hottest.active,0)} team slot${Math.max(hottest.capacity-hottest.active,0)===1?"":"s"} remain based on active registrations.`});
    }else if(hottest){
      items.push({tone:"info",label:"Capacity",title:`Highest demand: ${hottest.division.name}`,detail:`${hottest.active} of ${hottest.capacity} team slots are currently occupied (${hottest.utilization}%).`});
    }
    if(m.k.duprCoverage<70&&m.players.length){
      items.push({tone:"info",label:"Data quality",title:`DUPR coverage is ${m.k.duprCoverage}%`,detail:`${m.missingDupr} player${m.missingDupr===1?"":"s"} do not currently have a usable DUPR rating. Organizer review becomes more important for those players.`});
    }
    if(m.mixedLevel){
      items.push({tone:"info",label:"Level policy",title:`${m.mixedLevel} mixed-level team${m.mixedLevel===1?"":"s"} detected`,detail:"These teams are placed in the higher submitted level and remain subject to organizer approval."});
    }
    if(emptiest&&capacityRows.length>1&&emptiest.utilization<=25){
      items.push({tone:"muted",label:"Demand",title:`Low demand in ${emptiest.division.name}`,detail:`${emptiest.active} of ${emptiest.capacity} slots are occupied (${emptiest.utilization}%). Monitor whether bracket consolidation may eventually be needed.`});
    }
    if(!items.length){
      items.push({tone:"good",label:"Readiness",title:"No major operational exceptions in the current filter",detail:"Continue monitoring registration approvals, payment verification, and division capacity as new entries arrive."});
    }
    return items.slice(0,6);
  }

  function analystNarrativeMarkup(records){
    const m=analystMetrics(records);
    const capacityRows=[...m.capacity.rows].filter(r=>r.capacity).sort((a,b)=>b.utilization-a.utilization);
    const hottest=capacityRows[0];
    const topDivision=reportGroup(records,r=>(r.divisions||[]).map(id=>getDivision(id)?.name||id).join(" | ")||"Pending")[0];
    const scope=records.length===1?"1 registration":`${records.length.toLocaleString("en-PH")} registrations`;
    const approvalText=records.length?`${m.approvalRate}% approved and ${m.reviewRate}% still under review`:"no registrations in the current filter";
    const financeText=can("reports.financial")?` Verified collections stand at ${m.finance.collectionRate}% of expected active fees (${formatMoney(m.finance.verified)} verified against ${formatMoney(m.finance.expected)} expected).`:"";
    const capacityText=m.capacity.capacity?` Overall active capacity utilization is ${m.capacity.utilization}% (${m.capacity.active}/${m.capacity.capacity} team slots).`:"";
    const demandText=topDivision?` The strongest registration demand is currently ${topDivision.label} with ${topDivision.value} entr${topDivision.value===1?"y":"ies"}.`:"";
    const paceText=m.avgPerDay?` Registration volume averages ${m.avgPerDay.toFixed(1)} entr${m.avgPerDay===1?"y":"ies"} per active registration day.`:"";
    return `<section class="analyst-narrative-card">
      <div class="analyst-narrative-head"><div><span class="eyebrow">Management insights</span><h3>What the numbers are saying</h3></div><span class="analyst-scope-chip">${esc(scope)} in scope</span></div>
      <p><strong>Registration position:</strong> The current dataset is ${approvalText}.${capacityText}${demandText}</p>
      ${can("reports.financial")?`<p><strong>Financial position:</strong>${financeText} ${m.finance.awaitingCount?`${m.finance.awaitingCount} payment${m.finance.awaitingCount===1?"":"s"} are awaiting verification.`:"There are no payment records currently waiting for verification."}</p>`:""}
      <p><strong>Operational readiness:</strong> ${m.k.needsReview?`${m.k.needsReview} registration${m.k.needsReview===1?"":"s"} have unresolved eligibility, approval, or payment actions.`:"No current registrations are flagged for action."} DUPR coverage is ${m.k.duprCoverage}% and jersey production data is ${m.jerseyCompleteness}% complete.${paceText}</p>
      ${hottest?`<p><strong>Capacity watch:</strong> ${esc(hottest.division.name)} is the most utilized division at ${hottest.utilization}% (${hottest.active}/${hottest.capacity} active team slots).</p>`:""}
    </section>`;
  }

  function analystPriorityMarkup(records){
    const items=analystPriorityItems(records);
    return `<section class="analyst-priority-section">
      <div class="analyst-section-title"><div><span class="eyebrow">Management attention</span><h3>Priority Actions & Watchlist</h3><p>Items ranked for tournament management attention, based on the filtered registration data.</p></div></div>
      <div class="analyst-priority-grid">${items.map(item=>`<article class="analyst-priority ${esc(item.tone)}"><span>${esc(item.label)}</span><strong>${esc(item.title)}</strong><p>${esc(item.detail)}</p></article>`).join("")}</div>
    </section>`;
  }

  function analystInterpretation(key,records){
    const def=reportDefinition(key,records);
    const m=analystMetrics(records);
    const rows=def.rows||[];
    const top=rows[0];
    if(key==="trend"){
      if(!rows.length)return "No registration submissions fall inside the current filter, so a trend cannot yet be evaluated.";
      const total=rows.reduce((s,r)=>s+Number(r.value||0),0);
      const peak=[...rows].sort((a,b)=>b.value-a.value)[0];
      return `The filtered period contains ${total} registration${total===1?"":"s"}. The busiest submission date was ${peak?.label||"—"} with ${peak?.value||0} registration${peak?.value===1?"":"s"}. Use this trend to judge whether outreach or staffing needs to increase as registration progresses.`;
    }
    if(key==="status")return `${m.approved} of ${records.length} filtered registrations are approved (${m.approvalRate}%). ${m.underReview} remain under review. A growing under-review share indicates an approval backlog rather than demand weakness.`;
    if(key==="capacity"){
      const cap=[...m.capacity.rows].filter(r=>r.capacity).sort((a,b)=>b.utilization-a.utilization);
      const hot=cap[0], low=cap.at(-1);
      if(!hot)return "Configured division capacity is not available for the current dataset.";
      return `${hot.division.name} is the most utilized division at ${hot.utilization}%. ${low&&low!==hot?`${low.division.name} is the least utilized at ${low.utilization}%. `:""}The practical decision is whether any bracket is approaching closure while another may need consolidation or additional recruitment.`;
    }
    if(key==="division")return top?`${top.label} currently carries the highest demand with ${top.value} registration${top.value===1?"":"s"}. Compare this with capacity—not just raw counts—before opening, closing, or consolidating a division.`:"No assigned-division demand is available yet.";
    if(key==="verification")return `${m.eligibilityAction} registration${m.eligibilityAction===1?"":"s"} currently require eligibility attention. This workload should be cleared before final brackets and seeding are locked.`;
    if(key==="dupr")return `DUPR coverage is ${m.k.duprCoverage}% across ${m.players.length} known player${m.players.length===1?"":"s"}. Lower coverage increases reliance on organizer judgement, club references, and playing history for fair classification.`;
    if(key==="ratingBands")return top?`The largest DUPR band is ${top.label} with ${top.value} player${top.value===1?"":"s"}. Watch for concentration around level thresholds because borderline ratings can create more verification work.`:"No usable DUPR ratings are available in the current filter.";
    if(key==="financial")return `Verified collections are ${m.finance.collectionRate}% of expected active registration fees. ${formatMoney(m.finance.awaiting)} is awaiting verification and ${formatMoney(m.finance.verifiedOutstanding)} remains outstanding against verified collections. This is the most useful cash-control view for the Tournament Director.`;
    if(key==="paymentStatus")return `${m.finance.verifiedCount} active payment${m.finance.verifiedCount===1?" is":"s are"} verified, ${m.finance.awaitingCount} await verification, and ${m.finance.outstandingCount} remain outstanding. Operationally, separate “money received but unverified” from “not yet paid” so finance follow-up is targeted.`;
    if(key==="club")return top?`${top.label} is the largest represented club with ${top.value} player${top.value===1?"":"s"}. Club concentration can help with communications, but it should not influence eligibility or competitive placement.`:"Club affiliation data is not yet sufficient for a meaningful concentration view.";
    if(key==="ageGroups")return top?`The largest age segment is ${top.label} with ${top.value} player${top.value===1?"":"s"}. This is descriptive only and should not be used for skill placement unless the tournament has an explicit age-based division rule.`:"Age data is not yet sufficient for analysis.";
    if(key==="category")return top?`${top.label} currently has the largest category share with ${top.value} registration${top.value===1?"":"s"}. Use this with division capacity to identify categories that may fill earlier than others.`:"Category data is not yet available.";
    return def.description||"Use this view together with the detailed table to identify outliers, workload, and operational follow-up.";
  }

  function analystVisualCard(key,records,type=null,options={}){
    const def=reportDefinition(key,records);
    const visualType=type||def.defaultChart||"bar";
    const showTable=!!options.showTable;
    const tableLimit=Number(options.tableLimit||8);
    return `<article class="analyst-visual-card ${options.wide?"wide":""}">
      <div class="analyst-visual-head"><div><span class="eyebrow">${esc(options.kicker||"Analysis")}</span><h3>${esc(options.title||def.title)}</h3><p>${esc(def.description)}</p></div><span class="dashboard-visual-badge">${esc(dashboardChartLabel(visualType))}</span></div>
      <div class="analyst-chart-body">${reportChartMarkupForType(def,visualType,key)}</div>
      <div class="analyst-explanation"><span>Key takeaway</span><p>${esc(analystInterpretation(key,records))}</p></div>
      ${showTable?`<div class="analyst-mini-table">${dashboardCompactTable(def,tableLimit)}</div>`:""}
    </article>`;
  }

  function analystSummaryVisuals(records){
    return `<section class="analyst-section">
      <div class="analyst-section-title"><div><span class="eyebrow">Executive visuals</span><h3>Registration Health at a Glance</h3><p>Four decision-oriented visuals: pace, workflow, capacity, and cash collection.</p></div></div>
      <div class="analyst-visual-grid">
        ${analystVisualCard("trend",records,"line",{wide:true,kicker:"Demand pace"})}
        ${analystVisualCard("status",records,"doughnut",{kicker:"Workflow"})}
        ${analystVisualCard("capacity",records,"hbar",{wide:true,kicker:"Capacity"})}
        ${can("reports.financial")?analystVisualCard("paymentStatus",records,"doughnut",{kicker:"Finance"}):analystVisualCard("verification",records,"doughnut",{kicker:"Readiness"})}
      </div>
    </section>`;
  }

  function analystActionTable(records){
    const def=reportDefinition("actionRequired",records);
    return `<section class="analyst-section analyst-workqueue">
      <div class="analyst-section-title"><div><span class="eyebrow">Action queue</span><h3>Entries Requiring Follow-up</h3><p>This is the operational working list. Resolve these items before final approval and bracket handoff.</p></div><button class="button button-ghost" type="button" data-director-report="actionRequired">Open full action report</button></div>
      ${dashboardCompactTable(def,10)}
    </section>`;
  }

  function fullReportModule(title,subtitle,cards,records,tableKey=null){
    return `<section class="full-report-module">
      <div class="full-report-module-head"><div><span class="eyebrow">Full report</span><h3>${esc(title)}</h3><p>${esc(subtitle)}</p></div></div>
      <div class="analyst-visual-grid">${cards.map(card=>analystVisualCard(card.key,records,card.type,{wide:!!card.wide,kicker:card.kicker||"Analysis",title:card.title||null,showTable:!!card.showTable,tableLimit:card.tableLimit||8})).join("")}</div>
      ${tableKey?`<div class="full-report-table-block"><div class="full-report-table-head"><strong>Detailed working table</strong><button class="button button-ghost" type="button" data-director-report="${esc(tableKey)}">Open complete report</button></div>${dashboardCompactTable(reportDefinition(tableKey,records),12)}</div>`:""}
    </section>`;
  }

  function fullAnalystReportMarkup(records){
    const sections=[];
    let sectionNumber=1;

    if(can("reports.financial"))sections.push(fullReportModule(
      `${sectionNumber++}. Financial Performance & Reconciliation`,
      "Separates expected fees, submitted amounts, verified collections, and amounts requiring finance follow-up.",
      [
        {key:"financial",type:"bar",wide:true,kicker:"Financial position"},
        {key:"paymentStatus",type:"doughnut",kicker:"Payment workflow"},
        {key:"revenueMethod",type:"hbar",wide:true,kicker:"Collection channels"}
      ],records,"paymentReconciliation"
    ));

    sections.push(fullReportModule(
      `${sectionNumber++}. Registration Performance`,
      "Measures demand, submission pace, approval throughput, and the current workflow backlog.",
      [
        {key:"trend",type:"line",wide:true,kicker:"Registration pace"},
        {key:"status",type:"doughnut",kicker:"Approval workflow"},
        {key:"division",type:"hbar",wide:true,kicker:"Demand mix"},
        {key:"category",type:"doughnut",kicker:"Category mix"}
      ],records,"registrationSummary"
    ));
    sections.push(fullReportModule(
      `${sectionNumber++}. Division Demand & Capacity`,
      "Compares registration demand with configured team limits so the Director can see which brackets are filling fastest.",
      [
        {key:"capacity",type:"hbar",wide:true,kicker:"Utilization"},
        {key:"level",type:"bar",kicker:"Skill mix"},
        {key:"division",type:"hbar",wide:true,kicker:"Assigned teams"}
      ],records,"capacity"
    ));
    sections.push(fullReportModule(
      `${sectionNumber++}. Eligibility, Fairness & Classification`,
      "Focuses on level verification, DUPR coverage, mixed-level placement, and entries that still require organizer judgement.",
      [
        {key:"verification",type:"doughnut",kicker:"Validation workload"},
        {key:"dupr",type:"doughnut",kicker:"DUPR coverage"},
        {key:"ratingBands",type:"bar",wide:true,kicker:"Rating distribution"}
      ],records,"actionRequired"
    ));
    sections.push(fullReportModule(
      `${sectionNumber++}. Player Profile & Participation`,
      "Descriptive player composition for communications, operations, and participation analysis—not for skill classification unless explicitly defined by tournament rules.",
      [
        {key:"club",type:"hbar",wide:true,kicker:"Club representation"},
        {key:"gender",type:"doughnut",kicker:"Player composition"},
        {key:"ageGroups",type:"bar",wide:true,kicker:"Age profile"},
        {key:"ageVsDupr",type:"scatter",kicker:"Age / rating relationship"}
      ],records,"playersTeams"
    ));
    sections.push(fullReportModule(
      `${sectionNumber++}. Tournament Readiness & Production`,
      "Turns approved registration data into the practical lists needed for jersey production, check-in, and tournament handoff.",
      [
        {key:"verification",type:"doughnut",kicker:"Readiness"},
        {key:"status",type:"bar",kicker:"Approval status"}
      ],records,"tournamentRoster"
    ));
    return `<div class="full-analyst-report">
      ${sections.join("")}
      <section class="full-report-final-tables">
        <div class="full-report-module-head"><div><span class="eyebrow">Operational outputs</span><h3>Production & Governance Tables</h3><p>Use these as source-of-truth working lists for tournament execution and post-event review.</p></div></div>
        <div class="full-report-output-grid">
          ${["jersey","detail",...(can("access.manage")?["auditTrail"]:[]),...((can("notifications.manage")||can("access.manage"))?["emailDelivery"]:[])].map(key=>{
            const def=reportDefinition(key,records);
            return `<article><div><span>${esc(def.title)}</span><p>${esc(def.description)}</p></div><button class="button button-ghost" type="button" data-director-report="${esc(key)}">Open report</button></article>`;
          }).join("")}
        </div>
      </section>
    </div>`;
  ...
  function reportViewToggleMarkup(){
    return `<div class="report-view-toggle" role="tablist" aria-label="Reports view">
      <button type="button" class="${reportViewMode==="summary"?"active":""}" data-report-view="summary" role="tab" aria-selected="${reportViewMode==="summary"}"><span>Executive Summary</span><small>At-a-glance tournament health & priorities</small></button>
      <button type="button" class="${reportViewMode==="full"?"active":""}" data-report-view="full" role="tab" aria-selected="${reportViewMode==="full"}"><span>Detailed Report</span><small>Drill-down analysis, reconciliation & complete tables</small></button>
    </div>`;
  }

  function reportViewContextMarkup(){
    if(reportViewMode==="full"){
      return `<section class="report-view-context detailed-context">
        <div><span class="eyebrow">Detailed Report</span><h3>Drill down into the complete tournament dataset</h3><p>Filter registrations and review operational, financial, eligibility, participation, capacity, and production data in depth.</p></div>
        <span class="report-view-purpose">Analysis & reconciliation</span>
      </section>`;
    }
    return `<section class="report-view-context executive-context">
      <div><span class="eyebrow">Executive Summary</span><h3>Tournament health at a glance</h3><p>A concise management view of performance, outstanding actions, payment progress, capacity, and current tournament readiness.</p></div>
      <span class="report-view-purpose">Quick decision view</span>
    </section>`;
  }

  function renderAdminReports(){
    const allRecords=getRecords().filter(r=>!r.deleted);
    const records=reportViewMode==="summary"?allRecords:reportFilteredRecords();
    const k=reportKpis(records);
    ...
    const executiveSnapshot=executiveCapacitySnapshot(records);
    const executiveMetrics=executiveSnapshot.metrics;
    const executiveHot=executiveSnapshot.hottest;
    const reviewCount=records.filter(r=>workflowStatus(r.status)==="Under Review").length;
    const reviewRate=records.length?Math.round(reviewCount/records.length*100):0;
    const overviewKpis=`
      <div class="executive-kpi-grid" aria-label="Tournament executive summary metrics">
        ${executiveKpiCard("Total Registrations",k.registrations.toLocaleString("en-PH"),"Current tournament dataset","green","●","registrationSummary","__all__")}
        ${executiveKpiCard("Approved",k.approved.toLocaleString("en-PH"),`${records.length?Math.round(k.approved/records.length*100):0}% of total`,"yellow","✓","status","Approved")}
        ${executiveKpiCard("Under Review",reviewCount.toLocaleString("en-PH"),`${reviewRate}% of total`,"blue","◷","status","Under Review")}
        ${executiveKpiCard("Needs Attention",k.needsReview.toLocaleString("en-PH"),"Eligibility, approval, or payment","coral","!","actionRequired","__all__")}
        ${can("reports.financial")?executiveKpiCard("Verified Payments",formatMoney(k.verified),`${k.collectionRate}% of expected active fees`,"mint","▣","paymentStatus","Verified"):executiveKpiCard("DUPR Coverage",`${k.duprCoverage}%`,"Known players with DUPR","mint","D","dupr","Has DUPR")}
        ${executiveKpiCard("Capacity Utilization",`${executiveHot?.utilization??executiveMetrics.capacity?.utilization??0}%`,executiveHot?`${executiveHot.active}/${executiveHot.capacity} slots · ${executiveHot.division.name}`:`${k.assigned}/${k.capacity||"—"} assigned slots`,"violet","●","capacity",executiveHot?.division?.name||"__all__")}
      </div>`;

    const summaryContent=`
      ${overviewKpis}
      ${executiveOverviewMarkup(records)}
      ${analystPriorityMarkup(records)}
    `;

    const filterSection=`<section class="dashboard-filter-section analyst-filter-section">
      <div class="dashboard-filter-head">
        <div><span class="eyebrow">Analysis scope</span><h3>Filter the detailed report</h3><p>Every KPI, chart, explanation, and table in this view uses the same selected scope.</p></div>
        <span class="analyst-filter-count">${records.length.toLocaleString("en-PH")} registration${records.length===1?"":"s"}</span>
      </div>
    ...
    const fullContent=`
      ${filterSection}
      ${fullAnalystReportMarkup(records)}
      <details class="bi-explorer-details">
        <summary>
          <div><span class="eyebrow">Report explorer</span><strong>Custom Drill-down / Report Library</strong><small>Select any report, change the visual, and inspect the complete underlying table.</small></div>
        ...
        <section class="bi-explorer">
          <div class="bi-explorer-top">
            <div>
              ...
              <p>Use this for ad-hoc analysis beyond the recommended management report. The chart and table respond to the same page filters.</p>
            </div>
          ...
          <div class="bi-visual-card">
            ...
            <div class="analyst-explanation explorer-explanation"><span>Key takeaway</span><p>${esc(analystInterpretation(reportKey,records))}</p></div>
          </div>

          ${reportTableMarkup(def)}
        </section>
      </details>
      ${can("reports.financial")?`<section class="bi-finance-strip analyst-finance-strip">
        <div><span>Expected fees</span><strong>${formatMoney(k.expected)}</strong><small>Based on active registrations and configured division fees</small></div>
        <div><span>Verified collected</span><strong>${formatMoney(k.verified)}</strong><small>Payments marked Verified in registration records</small></div>
        <div><span>Verified balance outstanding</span><strong>${formatMoney(k.balance)}</strong><small>Expected active fees minus verified collections</small></div>
    ...
    return `<div class="bi-page analyst-report-page">
      <div class="admin-panel-heading bi-heading">
        <div>
          <p class="eyebrow">Reports & insights</p>
          <h2>Tournament Intelligence</h2>
          <p>Use Executive Summary for a fast management view, or Detailed Report for complete drill-down analysis, reconciliation, and tournament readiness.</p>
        </div>
      ...
      ${reportViewToggleMarkup()}
      ${reportViewContextMarkup()}
      ${reportViewMode==="full"?fullContent:summaryContent}
    </div>`;
  }
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  function renderAdminDuprRules(){
    ...
    return `<div class="admin-panel-heading"><div><p class="eyebrow">Level eligibility engine</p><h2>DUPR & Validation Rules</h2><p>These are tournament-defined DUPR thresholds. Players without DUPR may request a level, but the request remains provisional until the organizer validates the registration.</p></div><span class="status-pill">Organizer validation</span></div>
      ...
      <div class="admin-rule-note"><strong>Verification & pair rules</strong><p>DUPR players submit a rating with screenshot proof. No-DUPR players request a level and submit Club Affiliation and playing history. A supporting club/community link is optional. Approving the registration validates the requested level unless Admin changes it first. Partners may have different verified levels. When they differ, the higher verified level controls the team placement. Gender is limited to Male or Female and automatically determines Men's, Women's, or Mixed Doubles. Final placement remains subject to organizer verification before approval.</p></div>`;
  }
  ...
  function bindPanel(){
    if(adminTab==="registrations"){
      ...
      $$('[data-view-record]').forEach(control=>control.onclick=event=>{
        event.preventDefault();
        event.stopPropagation();
        openRecord(control.dataset.viewRecord,control.dataset.focusSection||"");
      });
    ...
    if(adminTab==="reports"){
      ...
      $$(`[data-report-view]`).forEach(btn=>btn.onclick=()=>{reportViewMode=btn.dataset.reportView==="full"?"full":"summary";renderAdmin()});
      ...
      $$('[data-director-report]').forEach(btn=>btn.onclick=()=>openDirectorReport(btn.dataset.directorReport));
    ...
    if(adminTab==="access"){
      ...
      const invite=$("#inviteAdminUserButton");if(invite)invite.onclick=openInviteUserDialog;
      $$("[data-own-password]").forEach(btn=>btn.onclick=()=>openOwnPasswordDialog({forced:false}));
      $$("[data-reset-admin-password]").forEach(btn=>btn.onclick=()=>resetOrganizerPassword(btn.dataset.resetAdminPassword));
      $$("[data-revoke-admin-user]").forEach(btn=>btn.onclick=()=>revokeOrganizerAccess(btn.dataset.revokeAdminUser));
      $$("[data-restore-admin-user]").forEach(btn=>btn.onclick=()=>restoreOrganizerAccess(btn.dataset.restoreAdminUser));
      return;
    }
    if(adminTab==="payments"){
      const save=$("#savePaymentMethodsButton");if(save)save.onclick=saveLivePaymentMethods;
      const add=$("#addPaymentMethodButton");if(add)add.onclick=addLivePaymentMethod;

      $$("[data-payment-method-accordion]").forEach(details=>{
        details.ontoggle=()=>{
          const id=String(details.dataset.paymentMethodAccordion||"");
          if(!id)return;
          if(details.open)openPaymentMethodIds.add(id);
          else openPaymentMethodIds.delete(id);
        };
      });
      ...
      $$("[data-payment-index]").forEach(input=>{
        const apply=()=>{
          ...
          if(field==="label"){
            const summary=document.querySelector(`[data-payment-summary-label="${m.id}"]`);
            if(summary)summary.textContent=String(value||"Payment method");
          }
          if(field==="enabled"){
            const badge=document.querySelector(`[data-payment-summary-status="${m.id}"]`);
            if(badge){
              badge.textContent=value?"Visible to players":"Hidden from players";
              badge.classList.toggle("is-enabled",!!value);
  ...
  function exportCsv(){
    ...
    getRecords().filter(r=>!r.deleted).forEach(r=>{
      ...
      rows.push([
        ...
        (e.classificationPending||e.partnerPending||e.categoryPending||!e.divisionId)?"Yes":"No",
  ...
  function renderAdminDuprRules(){
    ...
    return `<div class="admin-panel-heading"><div><p class="eyebrow">Level eligibility engine · Supabase live</p><h2>DUPR & Validation Rules</h2><p>These thresholds are shared by the Player portal and organizer verification workflow.</p></div><span class="status-pill">Server enforced</span></div>
      <div class="admin-form-grid">${adminDuprField("Low Intermediate starts at","duprEligibility.thresholds.lowIntermediateMin",t.lowIntermediateMin,"Ratings below this remain Beginner.")}${adminDuprField("High Intermediate starts at","duprEligibility.thresholds.highIntermediateMin",t.highIntermediateMin,"Must be higher than the Low Intermediate threshold.")}${adminDuprField("Advanced starts at","duprEligibility.thresholds.advancedMin",t.advancedMin,"Ratings at or above this threshold are Advanced.")}</div>
      <div class="admin-rule-note"><strong>Verification & pair rules</strong><p>DUPR players submit rating proof. Players without DUPR request a level, provide Club Affiliation and recent playing history, and may optionally add a supporting club/community link. Partners may have different verified levels; the higher verified level controls the team placement. Male/Male, Female/Female, and Male/Female automatically determine Men's, Women's, and Mixed Doubles.</p></div>`;
  }


  function isSuperAdmin(){
    return !!(supabaseAccessProfile?.is_super_admin||currentRole()?.isSuperAdmin);
  }
  ...
  async function loadLiveAdminAccounts({quiet=false}={}){
    if(liveAdminAccountsLoading)return liveAdminAccounts;
    liveAdminAccountsLoading=true;
    try{
      const response=await registrationAdminApi("admin-accounts-list");
      liveAdminAccounts=Array.isArray(response?.accounts)?response.accounts:[];
      liveAdminAccountsLoaded=true;
      liveAdminAccessCapabilities={
        ...
        canManagePasswords:!!response?.canManagePasswords,
        inviteRoles:Array.isArray(response?.inviteRoles)?response.inviteRoles:[]
      };
      return liveAdminAccounts;
    }catch(error){
      console.warn("Admin accounts:",error);
      liveAdminAccountsLoaded=false;
      if(!quiet)showToast(error?.message||"Organizer accounts could not be loaded");
      throw error;
    }finally{
      liveAdminAccountsLoading=false;
    }
  }

  function organizerAccountStatusBadge(account){
    if(account.status==="Revoked")return `<span class="organizer-account-status revoked">Revoked</span>`;
    if(account.mustChangePassword)return `<span class="organizer-account-status temporary">Temporary password</span>`;
    return `<span class="organizer-account-status active">Active</span>`;
  }

  function organizerAccountActions(account){
    const current=String(account.userId)===String(liveAdminAccessCapabilities.currentUserId);
    const superAdmin=liveAdminAccessCapabilities.isSuperAdmin;
    const protectedSuper=account.roleKey==="super_admin";

    if(current){
      return `<button class="button button-ghost organizer-account-action" type="button" data-own-password>Change My Password</button>`;
    }

    if(!superAdmin)return "";

    if(account.status==="Revoked"){
      return `<button class="button button-ghost organizer-account-action" type="button" data-restore-admin-user="${esc(account.userId)}">Restore Access</button>`;
    }

    return `<div class="organizer-account-action-group">
      <button class="button button-ghost organizer-account-action" type="button" data-reset-admin-password="${esc(account.userId)}">Issue Temporary Password</button>
      ${protectedSuper?"":`<button class="button button-danger organizer-account-action" type="button" data-revoke-admin-user="${esc(account.userId)}">Revoke Access</button>`}
    </div>`;
  }

  function liveOrganizerAccountsMarkup(){
    if(!liveAdminAccountsLoaded){
      return `<div class="access-live-loading"><span></span><div><strong>Loading organizer accounts…</strong><small>Reading server-authoritative account access from Supabase.</small></div></div>`;
    }
    if(!liveAdminAccounts.length){
      return `<div class="empty-state"><strong>No organizer accounts found.</strong>Run the Admin Security migration and reload this section.</div>`;
    }

    return `<div class="organizer-account-list">${liveAdminAccounts.map(account=>{
      const current=String(account.userId)===String(liveAdminAccessCapabilities.currentUserId);
      return `<article class="organizer-account-card ${account.status==="Revoked"?"is-revoked":""}">
        <div class="organizer-account-avatar">${esc((account.displayName||account.email||"U").slice(0,1).toUpperCase())}</div>
        <div class="organizer-account-main">
          <div class="organizer-account-title">
            <strong>${esc(account.displayName||account.email)}</strong>
            ${current?`<span class="organizer-you-badge">You</span>`:""}
            ${account.roleKey==="super_admin"?`<span class="organizer-super-badge">Super Admin</span>`:""}
          </div>
          <span>${esc(account.email)}</span>
          <div class="organizer-account-meta">
            <span>${esc(account.roleName||account.roleKey)}</span>
            ${organizerAccountStatusBadge(account)}
            ${account.invitedByEmail?`<small>Invited by ${esc(account.invitedByEmail)}</small>`:""}
          </div>
        </div>
        <div class="organizer-account-security">
          <small>${account.lastPasswordChangeAt?`Password changed ${esc(accessDate(account.lastPasswordChangeAt))}`:account.mustChangePassword?"Password change required":"Password managed by user"}</small>
          ${organizerAccountActions(account)}
        </div>
      </article>`;
    }).join("")}</div>`;
  }
  ...
  function openInviteUserDialog(){
    const dialog=$("#adminInviteUserDialog");
    const select=$("#adminInviteRole");
    if(!dialog||!select)return;

    const roles=liveAdminAccessCapabilities.inviteRoles||[];
    select.innerHTML=roles.map(role=>`<option value="${esc(role.key)}">${esc(role.name)}</option>`).join("");
    $("#adminInviteName").value="";
    $("#adminInviteEmail").value="";
    setSecurityStatus("#adminInviteUserStatus","");
    dialog.showModal();
    setTimeout(()=>$("#adminInviteName")?.focus(),40);
  }

  function setSecurityStatus(selector,message,type="error"){
    const el=$(selector);
    if(!el)return;
    el.hidden=!message;
    el.textContent=message||"";
  ...
  function openOwnPasswordDialog({forced=false}={}){
    const dialog=$("#adminOwnPasswordDialog");
    if(!dialog)return;
    ownPasswordDialogForced=!!forced;
    $("#adminOwnPassword").value="";
    $("#adminOwnPasswordConfirm").value="";
    $("#adminOwnPasswordShow").checked=false;
    setSecurityStatus("#adminOwnPasswordStatus","");

    const title=$("#adminOwnPasswordTitle");
    const eyebrow=$("#adminOwnPasswordEyebrow");
    const description=$("#adminOwnPasswordDescription");
    const close=$("#adminOwnPasswordClose");
    const cancel=$("#adminOwnPasswordCancel");

    if(forced){
      if(eyebrow)eyebrow.textContent="First-login security";
      if(title)title.textContent="Create Your Password";
      if(description)description.textContent="You signed in with a temporary password. Create your own password before accessing tournament data.";
      ...
      if(description)description.textContent="Only you can change your own password from this account.";
      if(close)close.hidden=false;
      if(cancel)cancel.hidden=false;
    }

    dialog.showModal();
    setTimeout(()=>$("#adminOwnPassword")?.focus(),50);
  }

  async function finalizeWorkspaceAfterForcedPassword(){
    const profile=await loadMyAdminAccess();
    adminTab=firstAccessibleTab();
    unlockAdmin();
    ...
    await loadLiveRegistrations({quiet:true});
    if(can("notifications.manage"))await loadProductionEmailSettings({quiet:true});
    if(can("access.manage"))await loadLiveAdminAccounts({quiet:true});
    renderAdmin();
    startLiveRegistrationSync();
    showToast("Password updated. Your organizer account is ready.");
    return profile;
  }

  async function saveOwnPassword(){
    const password=String($("#adminOwnPassword")?.value||"");
    const confirmation=String($("#adminOwnPasswordConfirm")?.value||"");
    const button=$("#adminOwnPasswordSave");

    if(password.length<8){
      setSecurityStatus("#adminOwnPasswordStatus","Use a password with at least 8 characters.");
      return;
    }
    if(password!==confirmation){
      setSecurityStatus("#adminOwnPasswordStatus","The two password entries do not match.");
      return;
    }
    ...
    try{
      ...
      await supabaseRequest("/functions/v1/registration-api",{
        method:"POST",
        body:{action:"admin-complete-password-change"},
        accessToken:supabaseSession.access_token
      });

      supabaseAccessProfile=await loadMyAdminAccess();
      $("#adminOwnPasswordDialog")?.close();

      if(ownPasswordDialogForced){
        ownPasswordDialogForced=false;
        await finalizeWorkspaceAfterForcedPassword();
      }else{
        if(adminTab==="access"){
          try{await loadLiveAdminAccounts({quiet:true})}catch{}
          renderAdmin();
        }
        showToast("Your password has been updated");
      }
    }catch(error){
      console.warn("Own password update:",error);
      setSecurityStatus("#adminOwnPasswordStatus",error?.message||"Password could not be updated.");
    }finally{
      if(button){button.disabled=false;button.textContent=ownPasswordDialogForced?"Create My Password":"Update My Password"}
    }
  }

  async function submitOrganizerInvite(){
    const button=$("#adminInviteUserSend");
    const displayName=String($("#adminInviteName")?.value||"").trim();
    const email=String($("#adminInviteEmail")?.value||"").trim().toLowerCase();
    const roleKey=String($("#adminInviteRole")?.value||"");

    if(!displayName||!email||!roleKey){
      setSecurityStatus("#adminInviteUserStatus","Complete the name, email and role.");
      return;
    }

    if(button){button.disabled=true;button.textContent="Sending Invitation…"}
    try{
      const response=await registrationAdminApi("admin-invite-user",{displayName,email,roleKey});
      liveAdminAccounts=Array.isArray(response?.accounts)?response.accounts:liveAdminAccounts;
      liveAdminAccountsLoaded=true;
      $("#adminInviteUserDialog")?.close();
      renderAdmin();
      showToast(`Invitation sent to ${email}`);
    }catch(error){
      console.warn("Organizer invite:",error);
      setSecurityStatus("#adminInviteUserStatus",error?.message||"Invitation could not be sent.");
    }finally{
      if(button){button.disabled=false;button.textContent="Send Invitation"}
    }
  }

  async function resetOrganizerPassword(userId){
    if(!liveAdminAccessCapabilities.isSuperAdmin)return;
    const account=liveAdminAccounts.find(row=>String(row.userId)===String(userId));
    if(!account)return;
    if(!confirm(`Issue a new temporary password to ${account.email}? Their current password will stop working, and they will be required to create a new one after login.`))return;

    try{
      const response=await registrationAdminApi("admin-reset-user-password",{userId});
      liveAdminAccounts=Array.isArray(response?.accounts)?response.accounts:liveAdminAccounts;
      renderAdmin();
      showToast(`Temporary password emailed to ${account.email}`);
    }catch(error){
      showToast(error?.message||"Password could not be reset");
    }
  }

  async function revokeOrganizerAccess(userId){
    if(!liveAdminAccessCapabilities.isSuperAdmin)return;
    const account=liveAdminAccounts.find(row=>String(row.userId)===String(userId));
    if(!account)return;
    if(!confirm(`Revoke all Animo Admin access for ${account.email}?`))return;

    try{
      const response=await registrationAdminApi("admin-revoke-user",{userId});
      liveAdminAccounts=Array.isArray(response?.accounts)?response.accounts:liveAdminAccounts;
      renderAdmin();
      showToast(`Access revoked for ${account.email}`);
    }catch(error){
      showToast(error?.message||"Access could not be revoked");
    }
  }

  async function restoreOrganizerAccess(userId){
    if(!liveAdminAccessCapabilities.isSuperAdmin)return;
    const account=liveAdminAccounts.find(row=>String(row.userId)===String(userId));
    if(!account)return;
    try{
      const response=await registrationAdminApi("admin-restore-user",{userId});
      liveAdminAccounts=Array.isArray(response?.accounts)?response.accounts:liveAdminAccounts;
      renderAdmin();
      showToast(`Access restored for ${account.email}`);
    }catch(error){
      showToast(error?.message||"Access could not be restored");
    }
  }

  function renderAdminAccess(){
    if(!can("access.manage"))return `<div class="empty-state"><strong>Access restricted.</strong>This section is available to Directors and the Super Admin.</div>`;

    if(!liveAdminAccountsLoaded&&!liveAdminAccountsLoading){
      setTimeout(async()=>{
        try{
          await loadLiveAdminAccounts({quiet:true});
          if(adminTab==="access")renderAdmin();
        }catch{}
      },0);
    }
    ...
    return `<div class="access-page access-live-page">
      <div class="admin-panel-heading">
        <div>
          <p class="eyebrow">${superAdmin?"Super Admin security":"Director access management"}</p>
          <h2>Access & Roles</h2>
          <p>Organizer accounts are enforced by Supabase Auth. Temporary-password accounts must create their own password before accessing tournament data.</p>
        </div>
        <div class="access-live-actions">
          <button class="button button-ghost" type="button" data-own-password>Change My Password</button>
          ${liveAdminAccessCapabilities.canInvite?`<button class="button button-primary" id="inviteAdminUserButton" type="button">+ Invite User</button>`:""}
        </div>
      </div>

      <div class="access-security-model">
        <article class="access-security-level super">
          <span>01</span>
          <div><strong>Super Admin</strong><p><b>qourtsph@gmail.com</b> is the protected Super Admin. Only this account can reset another user's password, revoke access, or restore revoked accounts.</p></div>
        </article>
        <article class="access-security-level director">
          <span>02</span>
          <div><strong>Director</strong><p>Directors can invite operational users. Only the Super Admin can invite another Director.</p></div>
        </article>
        <article class="access-security-level user">
          <span>03</span>
          <div><strong>All organizer users</strong><p>Every logged-in user can change only their own password. Invited users receive a temporary password and must replace it on first login.</p></div>
        </article>
      </div>

      <section class="access-live-accounts">
        <header>
          <div><p class="eyebrow">Server-authoritative accounts</p><h3>Organizer Users</h3></div>
          <span class="status-pill">${liveAdminAccountsLoaded?`${liveAdminAccounts.filter(a=>a.status==="Active").length} active`:"Loading…"}</span>
        </header>
        ${liveOrganizerAccountsMarkup()}
      </section>

      <details class="access-policy-details">
        <summary><div><strong>Role policy reference</strong><small>Advanced permission blueprint retained from the existing Admin.</small></div><span>⌄</span></summary>
        <div class="access-policy-details-body">
          <div class="access-backend-note"><strong>Live account enforcement now comes from the Admin Security layer.</strong><span>This advanced role-policy workspace is retained as a reference/configuration blueprint. Account creation, temporary passwords and revocation are handled by the live server controls above.</span></div>
  ...
  async function initializeAuthenticatedAdmin(){
    ...
    const recovery=recoveryHashPayload();
    if(recovery?.error){
      clearAuthHash();
      lockAdmin("");
      bindGlobal();
      authStatus(`Password-reset link error: ${recovery.error}`,"error");
      authInitializing=false;
      return;
    }
    if(recovery?.access_token){
      enterPasswordRecoveryMode(recovery);
      bindGlobal();
      authInitializing=false;
      return;
    }
  ...
  let liveSyncLifecycleBound=false;
  function bindLiveSyncLifecycle(){
    if(liveSyncLifecycleBound)return;
    liveSyncLifecycleBound=true;

    document.addEventListener("visibilitychange",()=>{
      if(document.hidden){
        stopLiveRegistrationSync();
      }else if(supabaseSession&&backendCompatible&&liveRegistrationLoaded){
        syncLiveRegistrationChanges();
      }
    });

    window.addEventListener("online",()=>{
      if(supabaseSession&&backendCompatible&&liveRegistrationLoaded){
        syncLiveRegistrationChanges();
      }
    });
    ...
    window.addEventListener("offline",()=>{
      stopLiveRegistrationSync();
      updateRegistrationDataStatus("LIVE SYNC PAUSED · Browser is offline.",true);
    });
  }


  async function navigateAdminExperience(next,{closeMore=true,scroll=true}={}){
    const needed=TAB_PERMISSIONS[next];
    if(needed&&!can(needed)){
      showToast("Your role does not have access to that section");
      return;
    }
    ...
    if(adminTab==="access"&&!liveAdminAccountsLoaded){try{await loadLiveAdminAccounts({quiet:true})}catch(e){}}

    if(closeMore){
      const more=$("#adminMobileMoreDialog");
      if(more?.open)more.close();
    }
    renderAdmin();
    if(scroll&&window.innerWidth<=1024)window.scrollTo({top:0,behavior:"smooth"});
  }

  function syncResponsiveAdminNavigation(){
    document.body.dataset.adminSection=adminTab;

    $$("[data-mobile-primary-tab]").forEach(btn=>{
      const tab=btn.dataset.mobilePrimaryTab;
      const needed=TAB_PERMISSIONS[tab];
      btn.hidden=needed?!can(needed):false;
      btn.classList.toggle("active",tab===adminTab);
      btn.setAttribute("aria-current",tab===adminTab?"page":"false");
    });

    const moreTabs=$$("[data-mobile-more-tab]");
    let secondaryActive=false;
    moreTabs.forEach(btn=>{
      const tab=btn.dataset.mobileMoreTab;
      const needed=TAB_PERMISSIONS[tab];
      btn.hidden=needed?!can(needed):false;
      const active=tab===adminTab;
      btn.classList.toggle("active",active);
      if(active)secondaryActive=true;
    });

    const moreButton=$("#adminMobileMoreButton");
    if(moreButton){
      moreButton.classList.toggle("active",secondaryActive);
      moreButton.setAttribute("aria-current",secondaryActive?"page":"false");
    }

    const select=$("#adminMobileNav");
    if(select)select.value=adminTab;
  }

  function bindGlobal(){
    bindLiveSyncLifecycle();
    if(globalUiBound)return;
    globalUiBound=true;
    $$(".admin-tab").forEach(btn=>{
      if(!btn.title)btn.title=btn.textContent.replace(/\s+/g," ").trim();
      btn.onclick=async()=>{
      const needed=TAB_PERMISSIONS[btn.dataset.adminTab];
    ...
    $$("[data-mobile-primary-tab]").forEach(btn=>btn.onclick=()=>navigateAdminExperience(btn.dataset.mobilePrimaryTab));
    $$("[data-mobile-more-tab]").forEach(btn=>btn.onclick=()=>navigateAdminExperience(btn.dataset.mobileMoreTab));

    const moreButton=$("#adminMobileMoreButton");
    const moreDialog=$("#adminMobileMoreDialog");
    const moreClose=$("#adminMobileMoreClose");
    if(moreButton&&moreDialog)moreButton.onclick=()=>moreDialog.showModal();
    if(moreClose&&moreDialog)moreClose.onclick=()=>moreDialog.close();
    if(moreDialog)moreDialog.addEventListener("click",event=>{
      if(event.target===moreDialog)moreDialog.close();
    });

    const changePassword=$("#adminChangePasswordButton");
    if(changePassword)changePassword.onclick=()=>openOwnPasswordDialog({forced:false});

    const ownPasswordForm=$("#adminOwnPasswordForm");
    if(ownPasswordForm)ownPasswordForm.onsubmit=event=>{event.preventDefault();saveOwnPassword()};
    const ownPasswordClose=$("#adminOwnPasswordClose");
    if(ownPasswordClose)ownPasswordClose.onclick=()=>{if(!ownPasswordDialogForced)$("#adminOwnPasswordDialog")?.close()};
    const ownPasswordCancel=$("#adminOwnPasswordCancel");
    if(ownPasswordCancel)ownPasswordCancel.onclick=()=>{if(!ownPasswordDialogForced)$("#adminOwnPasswordDialog")?.close()};
    const ownPasswordShow=$("#adminOwnPasswordShow");
    if(ownPasswordShow)ownPasswordShow.onchange=()=>{
      const type=ownPasswordShow.checked?"text":"password";
      if($("#adminOwnPassword"))$("#adminOwnPassword").type=type;
      if($("#adminOwnPasswordConfirm"))$("#adminOwnPasswordConfirm").type=type;
    };

    const inviteForm=$("#adminInviteUserForm");
    if(inviteForm)inviteForm.onsubmit=event=>{event.preventDefault();submitOrganizerInvite()};
    const inviteClose=$("#adminInviteUserClose");
    if(inviteClose)inviteClose.onclick=()=>$("#adminInviteUserDialog")?.close();
    const inviteCancel=$("#adminInviteUserCancel");
    if(inviteCancel)inviteCancel.onclick=()=>$("#adminInviteUserDialog")?.close();
    ...
    const recordModalClose=$("#recordModalClose");if(recordModalClose)recordModalClose.onclick=()=>{
      const modal=$("#recordModal");
      if(modal?._recordModalResizeHandler){
        window.removeEventListener("resize",modal._recordModalResizeHandler);
        modal._recordModalResizeHandler=null;
      }
      modal?.close();
    };
