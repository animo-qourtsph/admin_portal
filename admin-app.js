(() => {
  ...
  async function updateStatus(reference,status){
    ...
    try{
      const response=await registrationAdminApi("admin-update-status",{reference,status});
  ...
  function bindPanel(){
    if(adminTab==="registrations"){
      ...
      if(retry)retry.onclick=async()=>{try{await loadLiveRegistrations();renderAdmin()}catch(e){}};
      if(!liveRegistrationLoaded)return;
