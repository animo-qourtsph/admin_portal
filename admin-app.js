(() => {
  ...
  const RECORDS_KEY = `animo-registration-records-${config.tournamentId}-v25-fresh`;
  ...
  function getRecords(){
    if(liveRegistrationLoaded)return liveRegistrationRecords;
  ...
  function saveRecords(records){
    if(liveRegistrationLoaded){
      ...
      try{localStorage.setItem(RECORDS_KEY,JSON.stringify(liveRegistrationRecords))}catch(e){}
      return;
    }
  ...
  async function loadLiveRegistrations({quiet=false}={}){
    ...
    try{
      ...
      try{localStorage.setItem(RECORDS_KEY,JSON.stringify(liveRegistrationRecords))}catch(e){}
      updateRegistrationDataStatus(`Live Supabase records · ${liveRegistrationRecords.length} registration${liveRegistrationRecords.length===1?"":"s"} loaded.`);
      return liveRegistrationRecords;
    }catch(error){
      console.warn("Live registration load failed:",error);
      updateRegistrationDataStatus("Could not load live Supabase registrations. Showing the last local cache.",true);
      if(!quiet)showToast(error?.message||"Could not load live registration data");
      throw error;
    }finally{
  ...
  function renderTrash(){
    if(!liveTrashLoaded&&!liveTrashLoading){
      setTimeout(async()=>{try{await loadTrashRecords({quiet:true});if(adminTab==="trash")renderAdmin()}catch(e){}},0);
    }
  ...
  async function updateStatus(reference,status){
    ...
    const record=getRecords().find(r=>r.reference===reference);
    if(!record)return;
    ...
    try{
      ...
      if(Number(email.sent||0)>0){
        showToast(`${status} saved · ${email.sent} email${email.sent===1?"":"s"} sent · Player tracker updated`);
      }else if(Number(email.failed||0)>0){
        showToast(`${status} saved · tracker updated · email delivery needs attention`);
      }else if(Number(email.suppressed||0)>0){
        showToast(`${status} saved · tracker updated · email notification disabled`);
      }else{
        showToast(`${status} saved · Player tracker updated`);
      }
    }catch(error){
  ...
  function bindGlobal(){
    ...
    globalUiBound=true;
    $$(".admin-tab").forEach(btn=>btn.onclick=async()=>{const needed=TAB_PERMISSIONS[btn.dataset.adminTab];if(needed&&!can(needed)){showToast("Your role does not have access to that section");return}adminTab=btn.dataset.adminTab;if(adminTab==="trash"&&!liveTrashLoaded){try{await loadTrashRecords({quiet:true})}catch(e){}}renderAdmin()});
