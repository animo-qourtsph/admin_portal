...
const API_BUILD = "v12-payment-methods";
const BUCKET = "animo-registration-documents";
const PAYMENT_ASSETS_BUCKET = "animo-payment-assets";
...
const ALLOWED_PAYMENT_QR_TYPES = new Set(["image/jpeg", "image/png", "image/webp"]);
...
function adminVerificationShape(player: any) {
  return {
    ...
    duprProofAvailable: !!player?.dupr_proof_path,
...
function adminRegistrationShape(reg: any) {
  ...
  return {
    ...
    payment: {
      ...
      fileName: payment?.proof_path ? "Payment proof submitted" : "",
      proofAvailable: !!payment?.proof_path,
...
function paymentMethodClientShape(row: any, qrUrl = "") {
  return {
    id: row?.method_key ?? "",
    label: row?.label ?? "",
    enabled: !!row?.enabled,
...
async function paymentMethodRows(admin: any, tournamentId: string, enabledOnly = false) {
  let query = admin
    .from("payment_methods")
    .select("method_key,label,enabled,account_name,account_number,instructions,qr_path,sort_order,updated_at")
    .eq("tournament_id", tournamentId)
  ...
  if (error) throw new Error(`Payment methods could not be loaded: ${error.message}`);
  ...
  for (const row of data ?? []) {
    ...
    if (row.qr_path) {
      const { data: signed } = await admin.storage
        ...
        .createSignedUrl(row.qr_path, 3600);
      qrUrl = signed?.signedUrl || "";
    }
    methods.push(paymentMethodClientShape(row, qrUrl));
  }
  return methods;
}

function parsePaymentQrDataUrl(value: unknown) {
  const raw = clean(value, 8 * 1024 * 1024);
  if (!raw) return null;
  const match = raw.match(/^data:(image\/(?:png|jpeg|webp));base64,([A-Za-z0-9+/=\r\n]+)$/);
  if (!match) throw new Error("QR image must be PNG, JPG, or WEBP.");

  const mime = match[1];
  if (!ALLOWED_PAYMENT_QR_TYPES.has(mime)) throw new Error("QR image type is not supported.");

  const binary = atob(match[2].replace(/\s/g, ""));
  if (binary.length > 5 * 1024 * 1024) throw new Error("QR image exceeds the 5 MB limit.");
...
async function handlePublicPaymentMethods(ctx: any) {
  const tournament = await tournamentRow(ctx.supabaseAdmin);
  const methods = await paymentMethodRows(ctx.supabaseAdmin, tournament.id, true);
  return Response.json({ ok: true, methods, source: "supabase" });
}

async function handleAdminPaymentMethods(ctx: any) {
  await requireAdminPermission(ctx, "payments.verify");
  const tournament = await tournamentRow(ctx.supabaseAdmin);
  const methods = await paymentMethodRows(ctx.supabaseAdmin, tournament.id, false);
  return Response.json({ ok: true, methods, source: "supabase" });
}

async function handleAdminSavePaymentMethods(body: Json, ctx: any) {
  const profile = await requireAdminPermission(ctx, "payments.verify");
  const tournament = await tournamentRow(ctx.supabaseAdmin);
  const incoming = Array.isArray(body.methods) ? body.methods : [];
  if (!incoming.length) throw new Error("No payment methods were supplied.");

  const { data: existingRows, error: existingError } = await ctx.supabaseAdmin
    .from("payment_methods").select("method_key,qr_path").eq("tournament_id", tournament.id);
  if (existingError) throw new Error(existingError.message);

  const existingByKey = new Map((existingRows ?? []).map((row: any) => [row.method_key, row]));
  const savedKeys = [];

  for (let index = 0; index < incoming.length; index++) {
    const method = incoming[index] ?? {};
    const methodKey = clean(method.id || method.methodKey, 80).toLowerCase()
      .replace(/[^a-z0-9_-]+/g, "-").replace(/^-+|-+$/g, "");
    if (!methodKey) throw new Error(`Payment method ${index + 1} needs an ID.`);

    const label = requireText(method.label, `Payment method ${index + 1} label`, 120);
    const enabled = !!method.enabled;
    const accountName = clean(method.accountName, 180) || null;
    const accountNumber = clean(method.accountNumber, 180) || null;
    const instructions = clean(method.instructions, 3000) || null;
    const sortOrder = Number.isFinite(Number(method.sortOrder)) ? Math.trunc(Number(method.sortOrder)) : (index + 1) * 10;

    if (enabled && !accountNumber) throw new Error(`${label}: account number / payment identifier is required before enabling.`);

    const existing = existingByKey.get(methodKey);
    let qrPath = existing?.qr_path ?? null;
    let uploadedPath = null;

    if (method.removeQr === true && qrPath) {
      const { error } = await ctx.supabaseAdmin.storage.from(PAYMENT_ASSETS_BUCKET).remove([qrPath]);
      if (error) throw new Error(`${label}: QR image could not be removed: ${error.message}`);
      qrPath = null;
    }

    const qr = parsePaymentQrDataUrl(method.qrDataUrl);
    if (qr) {
      uploadedPath = `payment-methods/${methodKey}/qr-${crypto.randomUUID()}.${qr.extension}`;
      const { error } = await ctx.supabaseAdmin.storage.from(PAYMENT_ASSETS_BUCKET)
        .upload(uploadedPath, qr.bytes, { contentType: qr.mime, upsert: false });
      if (error) throw new Error(`${label}: QR image upload failed: ${error.message}`);

      const previousPath = qrPath;
      qrPath = uploadedPath;
      if (previousPath) {
        try { await ctx.supabaseAdmin.storage.from(PAYMENT_ASSETS_BUCKET).remove([previousPath]); } catch {}
      }
    }

    const { error: saveError } = await ctx.supabaseAdmin.from("payment_methods").upsert({
    ...
    }, { onConflict: "tournament_id,method_key" });

    if (saveError) throw new Error(`${label}: ${saveError.message}`);
    savedKeys.push(methodKey);
  }
  ...
  await ctx.supabaseAdmin.from("audit_logs").insert({
    ...
    details: { method_keys: savedKeys, actor_email: profile?.email ?? ctx?.user?.email ?? null },
  });

  const methods = await paymentMethodRows(ctx.supabaseAdmin, tournament.id, false);
  return Response.json({ ok: true, saved: savedKeys.length, methods });
}

async function handleAdminProofUrl(body: Json, ctx: any) {
  const reference = requireText(body.reference, "Registration reference", 40).toUpperCase();
  const proofType = requireText(body.proofType, "Proof type", 20);

  if (!["dupr", "payment"].includes(proofType)) {
    throw new Error("Unsupported proof type.");
  }

  if (proofType === "dupr") {
    await requireAdminPermission(ctx, "eligibility.verify");
  } else {
    await requireAdminPermission(ctx, "payments.view");
  }

  const tournament = await tournamentRow(ctx.supabaseAdmin);

  const { data: reg, error: regError } = await ctx.supabaseAdmin
    .from("registrations")
    .select(`
      id, reference,
      players ( player_slot, dupr_proof_path ),
      payments ( proof_path )
    `)
  ...
  if (proofType === "dupr") {
    const playerSlot = Number(body.playerSlot);
    if (![1, 2].includes(playerSlot)) {
      throw new Error("Player slot must be 1 or 2.");
    }

    const player = (reg.players ?? []).find((p: any) => Number(p?.player_slot) === playerSlot);
    storagePath = clean(player?.dupr_proof_path, 1000);

    if (!storagePath) {
      throw new Error(`Player ${playerSlot} does not have a DUPR proof upload.`);
    }
  } else {
    const payment = Array.isArray(reg.payments) ? reg.payments[0] : reg.payments;
    storagePath = clean(payment?.proof_path, 1000);

    if (!storagePath) {
      throw new Error("This registration does not have a payment proof upload.");
    }
  }
  ...
  if (signedError || !signed?.signedUrl) {
    throw new Error(`Proof could not be opened: ${signedError?.message || "signed URL unavailable"}`);
  }

  await ctx.supabaseAdmin.from("audit_logs").insert({
    tournament_id: tournament.id,
    action: proofType === "dupr"
      ? "registration.dupr_proof_viewed"
      : "registration.payment_proof_viewed",
    entity_type: "registration",
    entity_id: reg.id,
    details: {
      reference,
      proof_type: proofType,
      player_slot: proofType === "dupr" ? Number(body.playerSlot) : null,
...
async function handleAdminPermanentDelete(body: Json, ctx: any) {
  ...
  const playerRows = Array.isArray(reg.players)
    ? reg.players
    : (reg.players ? [reg.players] : []);

  const paymentRows = Array.isArray(reg.payments)
    ? reg.payments
    : (reg.payments ? [reg.payments] : []);

  const storagePaths = [
    ...playerRows.map((p: any) => p?.dupr_proof_path).filter(Boolean),
    ...paymentRows.map((p: any) => p?.proof_path).filter(Boolean),
  ];
  ...
  for (const table of ["email_events", "registration_status_history", "payments", "players"]) {
    const { error: childError } = await ctx.supabaseAdmin
      .from(table)
      .delete()
      .eq("registration_id", reg.id);

    if (childError) {
      throw new Error(`Permanent deletion stopped at ${table}: ${childError.message}`);
    }
  }
...
async function handleSubmit(req: Request, admin: any) {
  ...
  const paymentMethodKey = clean(payment.method, 80).toLowerCase();
  if (paymentMethodKey) {
    const { data: liveMethod, error: liveMethodError } = await admin
      .from("payment_methods")
      .select("method_key")
      .eq("tournament_id", tournament.id)
    ...
    if (liveMethodError || !liveMethod) {
      throw new Error("The selected payment method is no longer available. Return to Payment and choose an active method.");
    }
  }
  ...
  fetch: withSupabase({ auth: ["user", "publishable"] }, async (req, ctx) => {
    try {
      ...
      if (body.action === "payment-methods") {
        return await handlePublicPaymentMethods(ctx);
      }
      if (body.action === "admin-payment-methods") {
        return await handleAdminPaymentMethods(ctx);
      }
      if (body.action === "admin-payment-methods-save") {
        return await handleAdminSavePaymentMethods(body, ctx);
      }

      if (body.action === "admin-proof-url") {
        return await handleAdminProofUrl(body, ctx);
      }
