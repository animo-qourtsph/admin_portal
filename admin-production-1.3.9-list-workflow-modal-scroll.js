  ...
  function verificationActions(reference,slot,v){
    ...
    if(v?.hasDupr==="No"){
      if(can("eligibility.verify")&&!v?.manualLevelApproved){
        actions.push(`<button class="button button-verify-action" data-live-verify-level="${playerSlot}" data-record="${esc(reference)}" type="button">Verify Requested Level</button>`);
      }
