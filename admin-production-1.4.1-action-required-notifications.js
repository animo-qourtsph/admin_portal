(() => {
  ...
  function executiveKpiCard(label,value,help,tone="green",icon="●"){
    return `<article class="executive-kpi executive-kpi-${esc(tone)}"><div class="executive-kpi-icon" aria-hidden="true">${icon}</div><div><span>${esc(label)}</span><strong>${value}</strong><small>${esc(help)}</small></div></article>`;
  }
    ...
    return `<article class="executive-visual-card executive-payment-card">
      ...
      <div class="executive-ring-layout">
        ...
        <div class="executive-ring-legend">
          <div><i class="legend-dot verified"></i><span>Verified</span><strong>${rate}%</strong><small>${formatMoney(f.verified)}</small></div>
          <div><i class="legend-dot pending"></i><span>Pending / outstanding</span><strong>${Math.max(100-rate,0)}%</strong><small>${formatMoney(Math.max(f.expected-f.verified,0))}</small></div>
        </div>
      </div>
    ...
    return `<article class="executive-visual-card executive-capacity-card">
      ...
      <div class="executive-ring-layout">
        ...
        <div class="executive-ring-legend single">
          <div><i class="legend-dot verified"></i><span>Filled</span><strong>${active}</strong><small>${esc(name)}</small></div>
          <div><i class="legend-dot pending"></i><span>Available</span><strong>${cap?Math.max(cap-active,0):"—"}</strong><small>Remaining team slots</small></div>
        </div>
      </div>
    ...
    return `<div class="executive-dashboard-overview">
      <div class="executive-visual-grid">
        <article class="executive-visual-card">
          ...
          <div class="executive-chart-body executive-status-chart">${reportChartMarkupForType(statusDef,"doughnut")}</div>
        ...
        <article class="executive-visual-card">
          ...
          <div class="executive-chart-body executive-division-chart">${reportChartMarkupForType(divisionDef,"hbar")}</div>
  ...
  function analystVisualCard(key,records,type=null,options={}){
    ...
    return `<article class="analyst-visual-card ${options.wide?"wide":""}">
      ...
      <div class="analyst-chart-body">${reportChartMarkupForType(def,visualType)}</div>
  ...
  function renderAdminReports(){
    ...
    const overviewKpis=`
      <div class="executive-kpi-grid" aria-label="Tournament executive summary metrics">
        ${executiveKpiCard("Total Registrations",k.registrations.toLocaleString("en-PH"),"Current tournament dataset","green","●")}
        ${executiveKpiCard("Approved",k.approved.toLocaleString("en-PH"),`${records.length?Math.round(k.approved/records.length*100):0}% of total`,"yellow","✓")}
        ${executiveKpiCard("Under Review",reviewCount.toLocaleString("en-PH"),`${reviewRate}% of total`,"blue","◷")}
        ${executiveKpiCard("Needs Attention",k.needsReview.toLocaleString("en-PH"),"Eligibility, approval, or payment","coral","!")}
        ${can("reports.financial")?executiveKpiCard("Verified Payments",formatMoney(k.verified),`${k.collectionRate}% of expected active fees`,"mint","▣"):executiveKpiCard("DUPR Coverage",`${k.duprCoverage}%`,"Known players with DUPR","mint","D")}
        ${executiveKpiCard("Capacity Utilization",`${executiveHot?.utilization??executiveMetrics.capacity?.utilization??0}%`,executiveHot?`${executiveHot.active}/${executiveHot.capacity} slots · ${executiveHot.division.name}`:`${k.assigned}/${k.capacity||"—"} assigned slots`,"violet","●")}
      </div>`;
