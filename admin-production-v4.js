(() => {
  ...
  async function checkBackendCompatibility(){
    try{
      ...
      if(response?.schemaReady===false){
        ...
        backendCheckError="Supabase schema is incomplete. Required migrations must be applied before Admin operations can continue.";
  ...
  function renderRegistrations(){
    ...
    return `<div class="admin-panel-heading registration-heading">
      ...
      <div class="admin-stat-grid">
        ...
        ${statCard("New / submitted",s.submitted)}
        ${statCard("Pending",s.pending)}
        ${statCard("Approved",s.approved)}
        ${statCard("Confirmed",s.confirmed)}
      </div>
  ...
  function registrationListRow(r){
    ...
    return `<tr class="registration-list-row" data-row-reference="${esc(r.reference)}">
      <td>
      ...
      <td>
        <div class="registration-list-players">
          <strong>${esc(p1)}</strong>
          <span>${esc(p2)}</span>
        </div>
      </td>
      <td><span class="registration-list-division">${esc(divs)}</span></td>
      <td>
      ...
      <td>
        ${canAny("registrations.status","registrations.approve","registrations.confirm","registrations.reject")
          ? `<select class="registration-list-status" data-status-record="${esc(r.reference)}" aria-label="Update ${esc(r.reference)} status">${permittedStatusOptions(r).map(s=>`<option ${s===r.status?"selected":""}>${s}</option>`).join("")}</select>`
          : `<span class="status-pill">${esc(r.status||"Submitted")}</span>`}
      </td>
      <td><span class="registration-list-date">${esc(submitted)}</span></td>
      <td class="registration-list-actions">
  ...
  function openRecord(reference){
    const r=getRecords().find(x=>x.reference===reference);
    if(!r)return;
    ...
    $("#recordModalBody").innerHTML=`
      <div class="record-modal-shell">
            ...
            <div class="record-payment-layout">
              <div>
                <div class="record-detail-grid payment-detail-grid">
                  ...
                  ${detail("Amount paid",r.payment?.amountPaid?formatMoney(r.payment.amountPaid):"—")}
        ...
        <footer class="record-modal-actions">
          <div class="record-modal-danger-actions">
            ${can("data.delete")?`<button class="button button-ghost danger-text" data-trash-record="${esc(r.reference)}" type="button">Move to Trash</button>`:""}
          </div>
          <div class="record-modal-status-actions">
            ${can("registrations.status")?`<button class="button button-ghost" data-set-pending="${esc(r.reference)}" type="button">Mark Pending</button>`:""}
            ${can("registrations.reject")?`<button class="button button-deny" data-set-rejected="${esc(r.reference)}" type="button">Deny</button>`:""}
            ${can("registrations.approve")?`<button class="button button-primary" data-set-approved="${esc(r.reference)}" type="button">Approve</button>`:""}
            ${can("registrations.confirm")?`<button class="button button-primary" data-set-confirmed="${esc(r.reference)}" type="button">Confirm</button>`:""}
          </div>
        </footer>
      </div>`;
    ...
    if(verifyPayment)verifyPayment.onclick=()=>liveVerifyPayment(reference);

    const trashButton=$("[data-trash-record]",modal);
    if(trashButton)trashButton.onclick=()=>trashRegistration(reference);

    const pendingBtn=$("[data-set-pending]",modal);
    if(pendingBtn)pendingBtn.onclick=()=>{updateStatus(reference,"Pending");modal.close()};

    const denyBtn=$("[data-set-rejected]",modal);
    if(denyBtn)denyBtn.onclick=()=>{denyRegistration(reference);modal.close()};

    const approveBtn=$("[data-set-approved]",modal);
    if(approveBtn)approveBtn.onclick=()=>{updateStatus(reference,"Approved");modal.close()};

    const confirmBtn=$("[data-set-confirmed]",modal);
    if(confirmBtn)confirmBtn.onclick=()=>{updateStatus(reference,"Confirmed");modal.close()};

    $$("[data-live-verify-dupr]",modal).forEach(btn=>btn.onclick=async()=>liveVerifyPlayer(reference,Number(btn.dataset.liveVerifyDupr),"verify-dupr"));
    $$("[data-live-request-proof]",modal).forEach(btn=>btn.onclick=async()=>liveVerifyPlayer(reference,Number(btn.dataset.liveRequestProof),"request-dupr-proof"));
    $$("[data-live-correct-dupr]",modal).forEach(btn=>btn.onclick=async()=>liveCorrectDupr(reference,Number(btn.dataset.liveCorrectDupr)));
    $$("[data-live-verify-level]",modal).forEach(btn=>btn.onclick=async()=>liveVerifyNoDuprLevel(reference,Number(btn.dataset.liveVerifyLevel)));
    $$("[data-live-change-level]",modal).forEach(btn=>btn.onclick=async()=>liveChangeNoDuprLevel(reference,Number(btn.dataset.liveChangeLevel)));

    const cat=$("[data-set-category]",modal);
    if(cat)cat.onclick=()=>assignCategory(reference);
  }
  ...
  function denialReason(){
    const raw=prompt("Reason for denying this registration:\n\nThis reason will be recorded and included in the player email.");
    if(raw===null)return null;
    const reason=String(raw).trim();
    if(!reason){showToast("A reason is required to deny a registration");return null}
    return reason;
  }

  async function denyRegistration(reference){
    const reason=denialReason();
    if(!reason)return;
    return updateStatus(reference,"Rejected",{reason});
  }
  ...
  function reportDefinition(key,records){
    ...
    if(key==="detail"){
      ...
      return {
        ...
        defaultChart:"table",
        tableOnly:true,
        tableHeaders:["Reference","Status","Player 1","Player 2","Division","Level","Category","Verification","Player 1 Club","Player 2 Club","Player 1 Jersey Back Name","Player 2 Jersey Back Name","Payment Method","Amount Paid","Submitted"],
        tableRows:rows
      };
    }
  ...
  function exportCsv(){
    ...
    const rows=[["Reference","Status","Division","Team Level","Category","Level Mismatch","Classification Pending","Organizer Verification","Player 1","Player 1 DOB","Player 1 Gender","Player 1 Club","Player 1 DUPR","Player 1 Requested Level","Player 1 Validated Level","Player 1 Club DUPR / Facebook Link","Player 1 Proof File","Player 1 Verification Status","Player 1 Playing Background","Player 1 Jersey Back Name","Player 2","Player 2 DOB","Player 2 Gender","Player 2 Club","Player 2 DUPR","Player 2 Requested Level","Player 2 Validated Level","Player 2 Club DUPR / Facebook Link","Player 2 Proof File","Player 2 Verification Status","Player 2 Playing Background","Player 2 Jersey Back Name","Payment Hold","Payment Method","Amount Paid","Submitted At"]];
