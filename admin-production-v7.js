(() => {
      ...
      <div class="director-card-header">
        <div class="director-card-ident">
          <div class="director-card-badges">
            <span class="status-pill">${esc(r.status||"Submitted")}</span>
            ${needsAction
              ? `<span class="director-action-pill">Action needed</span>`
              : `<span class="director-ready-pill">Ready</span>`}
          </div>
        ...
        <div class="director-card-controls">
          ...
          ${canAny("registrations.status","registrations.approve","registrations.confirm","registrations.reject")
            ? `<select data-status-record="${esc(r.reference)}" aria-label="Update registration status">${permittedStatusOptions(r).map(s=>`<option ${s===r.status?"selected":""}>${s}</option>`).join("")}</select>`
            : ""}
        </div>
      </div>
  ...
  function openRecord(reference){
    ...
    if(pendingBtn)pendingBtn.onclick=()=>{updateStatus(r.reference,"Pending");modal.close()};

    const denyBtn=$("[data-set-rejected]",modal);
    if(denyBtn)denyBtn.onclick=()=>{denyRegistration(r.reference);modal.close()};

    const approveBtn=$("[data-set-approved]",modal);
    if(approveBtn)approveBtn.onclick=()=>{updateStatus(r.reference,"Approved");modal.close()};

    const confirmBtn=$("[data-set-confirmed]",modal);
    if(confirmBtn)confirmBtn.onclick=()=>{updateStatus(r.reference,"Confirmed");modal.close()};
  ...
  function bindPanel(){
    if(adminTab==="registrations"){
      ...
      $$('[data-status-record]').forEach(sel=>sel.onchange=event=>{
        event.stopPropagation();
        updateStatus(sel.dataset.statusRecord,sel.value);
      });
      return;
    }
    if(adminTab==="trash"){
